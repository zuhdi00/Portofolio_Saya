...
// CORS and Private Network Access headers
...
if ($_SERVER['REQUEST_METHOD'] == 'OPTIONS') {
    // Allow browser preflight for CORS/PNA
...
try {
    ...
    $sql = "SELECT " . $selectCore . $selectExtra . " FROM tbOP op " . $joinExtra . " WHERE EXISTS (SELECT 1 FROM tbSC sc WHERE sc.cNoSC = op.cNoSc)";
    ...
    $countSql = "SELECT COUNT(*) as total FROM tbOP op WHERE op.cNoMc IS NOT NULL AND EXISTS (SELECT 1 FROM tbSC sc WHERE sc.cNoSC = op.cNoSc)";
    if (!empty($whereConditions)) {
    ...
    $orderClause = "ORDER BY op.dTglKirim2 DESC, op.cNoOp DESC";
