...
try {
    ...
    $sqlOrder = "
        SELECT
            ...
            CAST(SUM(ISNULL(t.nTot_brutto, 0) * ISNULL(t.nQty, 0)) / 1000.0 AS DECIMAL(18,5)) AS total_box_ton,
            CAST(SUM(ISNULL(d.nBrt, 0) * ISNULL(d.nQty, 0)) / 1000.0 AS DECIMAL(18,5)) AS total_perlengkapan_ton,
            CAST((
                SUM(ISNULL(t.nTot_brutto, 0) * ISNULL(t.nQty, 0)) +
                SUM(ISNULL(d.nBrt, 0) * ISNULL(d.nQty, 0))
            ) / 1000.0 AS DECIMAL(18,5)) AS total_order_ton
        FROM tbTSC t
        LEFT JOIN tbTSCDtl d ON t.cNoTSC = d.cNoTSC
    ...
    $totMap   = [];  // total_order (box + perlengkapan, langsung dari SQL)
    while ($row = sqlsrv_fetch_array($stmtOrder, SQLSRV_FETCH_ASSOC)) {
        $tgl = $row['tanggal_date'];
        if ($tgl instanceof DateTime) $key = $tgl->format('Y-m-d');
        else $key = date('Y-m-d', strtotime((string)$tgl));

        $orderMap[$key] = (float)$row['total_box_ton'];
        $perlMap[$key]  = (float)$row['total_perlengkapan_ton'];
        $totMap[$key]   = (float)$row['total_order_ton'];
    }
    ...
    foreach ($period as $dt) {
        ...
        // total order = total box + total perlengkapan (dari SQL langsung)
        $dayOrder = isset($totMap[$key])  ? round((float)$totMap[$key],   5) : 0.0;
