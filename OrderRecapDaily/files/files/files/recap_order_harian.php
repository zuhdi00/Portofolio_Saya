...
try {
    ...
    // 1. Kuota dari tbSetting (Nilai dalam ton)
    $kuota_ton = 0.0;
    ...
    if ($stmtKuota !== false) {
        ...
        if ($rowK && isset($rowK['nilai_raw'])) {
            ...
            $kuota_ton = is_numeric($clean) ? (float)$clean : 0.0;
        }
        sqlsrv_free_stmt($stmtKuota);
    }

    // 2. Recap order harian dengan filter tanggal
    //    GROUP BY CAST(Dtgl AS DATE) agar datetime/smalldatetime tetap per hari
    $sqlOrder = "
        SELECT
            CAST(t.dexpired2 AS DATE) AS tanggal_date,
            CAST(SUM(t.nTot_netto * t.nQty) / 1000.0 AS DECIMAL(10,2)) AS total_order_ton
        FROM tbTSC t
    ...
    foreach ($period as $dt) {
        ...
        $orderDisplay = $dayOrder;
        $rvDisplay = $rvVal;
        if ($dayOrder > $kuota_ton) {
            $excess = $dayOrder - $kuota_ton;
            $orderDisplay = $kuota_ton;
            $rvDisplay += $excess;
        }

        $dayContribution = round($orderDisplay + $rvDisplay, 4);
        ...
        $rowsAsc[] = [
            ...
            'total_order_ton'  => number_format($orderDisplay, 2, '.', '') . ' Ton',
            'penambahan_order' => number_format($rvDisplay, 2, '.', '') . ' Ton',
