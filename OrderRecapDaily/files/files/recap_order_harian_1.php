...
try {
    ...
    $sqlKuota = "SELECT TOP 1 CAST(Nilai / 1000.0 AS DECIMAL(10,2)) AS kuota_ton
                 FROM tbSetting
                 WHERE KodeSetting = 'NilaiMaxOrderPerHari'";   -- ganti cKode sesuai data aktual
    ...
    // ---------------------------------------------------------------
    $sqlOrder = "SELECT
                    FORMAT(t.tbDtgl, 'dd/MM/yyyy') AS tanggal,
                    t.tbDtgl AS tanggal_raw,
                    CAST(SUM(t.nTot_brutto * t.nQty) / 1000.0 AS DECIMAL(10,2)) AS total_order_ton
                 FROM tbTSC t
                 WHERE t.tbDtgl >= DATEADD(DAY, -30, GETDATE())
                 GROUP BY FORMAT(t.tbDtgl, 'dd/MM/yyyy'), t.tbDtgl
                 ORDER BY t.tbDtgl ASC";
