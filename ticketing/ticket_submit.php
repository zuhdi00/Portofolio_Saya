...
// Capture raw body and headers for debugging
$rawBody = file_get_contents('php://input');
// helper to extract headers in a portable way
...
logMsg('Incoming request headers: ' . json_encode(getRequestHeaders()));
logMsg('Raw body (len=' . strlen($rawBody) . '): ' . substr($rawBody,0,2000));
if (stripos($contentType, 'application/json') !== false) {
    $body = $rawBody !== false ? $rawBody : file_get_contents('php://input');
...
if ($name === '' || $dept === '' || $subject === '' || $desc === '') {
    // Log details to help diagnose production 400
    logMsg('Validation failed - missing fields. POST keys: ' . json_encode(array_keys($_POST)) . ' | parsed input keys: ' . json_encode(array_keys((array)$input)));
