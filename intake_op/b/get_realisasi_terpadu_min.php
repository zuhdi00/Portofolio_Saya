...
if (!in_array($action, ['list','mc_suggest','update_status'])) {
    echo json_encode(['success'=>false,'message'=>'only list, mc_suggest, and update_status supported']); exit;
}

$conn = dbConnect($serverName, $connectionOptions);

// update_status: set tbSC.cStatus (and cBatalStatus when closing) for a given cNoSc
if ($action === 'update_status') {
    $cNoSc  = trim($_REQUEST['sc_no'] ?? '');
    $status = trim($_REQUEST['status'] ?? ''); // 'OPEN' or 'CLOSE'
    if ($cNoSc === '' || !in_array($status, ['OPEN','CLOSE'])) {
        echo json_encode(['success'=>false,'message'=>'sc_no and a valid status (OPEN/CLOSE) are required']);
        sqlsrv_close($conn); exit;
    }

    if ($status === 'CLOSE') {
        $usql = "UPDATE tbSC SET cStatus = 'CLOSE', cBatalStatus = '1' WHERE cNoSc = ?";
    } else {
        // Reopen: restore status and clear the cancel flag
        $usql = "UPDATE tbSC SET cStatus = 'OPEN', cBatalStatus = '0' WHERE cNoSc = ?";
    }

    $ustmt = sqlsrv_query($conn, $usql, [$cNoSc], ["QueryTimeout"=>60]);
    if ($ustmt === false) {
        $errs = sqlsrv_errors();
        sqlsrv_close($conn);
        echo json_encode(['success'=>false,'message'=>'Update failed','errors'=>$errs], JSON_UNESCAPED_UNICODE);
        exit;
    }
    sqlsrv_free_stmt($ustmt);
    sqlsrv_close($conn);
    echo json_encode(['success'=>true,'message'=>"Status SC $cNoSc diubah menjadi $status",'cNoSc'=>$cNoSc,'status'=>$status], JSON_UNESCAPED_UNICODE);
    exit;
}
...
$sql = "SELECT
    ...
    ISNULL(sc.cBatalStatus, '0') AS sc_batal_status,
...
$where = [];
$params = [];
// Default rule: hide rows whose SC status is CLOSE (cStatus='CLOSE' / cBatalStatus='1').
// Rows with no matching tbSC (NULL) are treated as OPEN and remain visible.
