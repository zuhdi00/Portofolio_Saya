...
// Selalu tarik semua data (tanpa OFFSET/FETCH).
// Kirim ?limit=N untuk paginasi eksplisit jika diperlukan di masa depan.
$limitRaw = $_GET['limit'] ?? null;
if ($limitRaw !== null && $limitRaw !== 'all' && $limitRaw !== '0' && intval($limitRaw) > 0) {
    $limit  = min(1000000, intval($limitRaw));
    $offset = max(0, intval($_GET['offset'] ?? 0));
} else {
    $limit  = 0; // 0 = tanpa limit, tidak memakai OFFSET/FETCH
    $offset = 0;
}
...
$sql = "SELECT
    ...
    -- Tanggal kirim efektif: UserDate jika terisi & lebih relevan, fallback dTanggal SRJ
    ISNULL(srj_agg.tgl_kirim_eff, srj_agg.tgl_kirim_srj) AS tgl_kirim_eff,
...
-- SRJ aggregat: per cNoOp+cNoScDtl+cNoSC, sertakan dTanggal (tbSRJ) DAN UserDate (tbSRJDtl)
-- tgl_kirim_srj  = MAX(dTanggal)   → tanggal dokumen SRJ terakhir
-- userdate_srj   = MAX(UserDate)   → tanggal aktual kirim (lebih akurat jika diisi)
-- tgl_kirim_eff  = nilai terbesar antara dTanggal dan UserDate → acuan filter/display utama
LEFT JOIN (
    SELECT
        d.cNoOp                                         AS cNoOp,
        MAX(d.cNoScDtl)                                 AS cNoScDtl,
        MAX(s.cNoSC)                                    AS cNoSC,
        SUM(ISNULL(d.nQty,0))                           AS jml_kirim,
        -- Tanggal dokumen SRJ (header)
        MAX(s.dTanggal)                                 AS tgl_kirim_srj,
        -- UserDate per baris detail (tanggal aktual kirim)
        MAX(d.UserDate)                                 AS userdate_srj,
        -- Tanggal efektif = yang paling akhir dari keduanya (GREATEST via CASE)
        MAX(CASE
                WHEN ISNULL(d.UserDate, s.dTanggal) > s.dTanggal
                THEN ISNULL(d.UserDate, s.dTanggal)
                ELSE s.dTanggal
            END)                                        AS tgl_kirim_eff,
        MAX(s.cTujuanKirim)                             AS tujuan_kirim,
        SUM(CASE
                WHEN COALESCE(vw.lVoid, s.lVoid, '0') = '1'
                THEN 0
    ...
    GROUP BY d.cNoOp, d.cNoScDtl, s.cNoSC
        ...
        srj_agg.cNoOp    = op.cNoOp
     OR srj_agg.cNoScDtl = sc.cNoSc
     OR srj_agg.cNoSC    = sc.cNoSc
)
...
// Filter tanggal kirim: gunakan srj_agg.tgl_kirim_eff (gabungan dTanggal tbSRJ + UserDate tbSRJDtl)
// Karena srj_agg sudah di-JOIN di SQL, cukup filter kolom alias — tanpa EXISTS subquery tambahan.
if (!empty($shipFrom) && !empty($shipTo)) {
    // Rentang penuh: efektif kirim harus masuk antara shipFrom dan shipTo
    $where[] = "(srj_agg.tgl_kirim_eff >= ? AND srj_agg.tgl_kirim_eff <= ?)";
    $params[] = $shipFrom;
    $params[] = $shipTo . ' 23:59:59';
} elseif (!empty($shipFrom)) {
    $where[] = "srj_agg.tgl_kirim_eff >= ?";
    $params[] = $shipFrom;
} elseif (!empty($shipTo)) {
    $where[] = "srj_agg.tgl_kirim_eff <= ?";
    $params[] = $shipTo . ' 23:59:59';
...
// Count query — harus sinkron dengan main query (sertakan srj_agg jika ada ship filter)
$needSrjAgg = !empty($shipFrom) || !empty($shipTo);
...
if ($needSrjAgg) {
    $countSql .= "
LEFT JOIN (
    SELECT d.cNoOp, MAX(d.cNoScDtl) AS cNoScDtl, MAX(s.cNoSC) AS cNoSC,
           MAX(CASE WHEN ISNULL(d.UserDate, s.dTanggal) > s.dTanggal
                    THEN ISNULL(d.UserDate, s.dTanggal) ELSE s.dTanggal END) AS tgl_kirim_eff
    FROM tbSRJDtl d WITH (NOLOCK)
    INNER JOIN tbSRJ s WITH (NOLOCK) ON s.cNoSRJ = d.cNoSRJ
    GROUP BY d.cNoOp, d.cNoScDtl, s.cNoSC
) srj_agg ON (srj_agg.cNoOp = op.cNoOp OR srj_agg.cNoScDtl = sc.cNoSc OR srj_agg.cNoSC = sc.cNoSc)";
}
$countSql .= "\nWHERE 1=1";
...
if ($stmt !== false) {
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        ...
        // Label tanggal kirim aktual dari SRJ (UserDate jika ada, fallback dTanggal SRJ)
        $r['tgl_kirim_aktual'] = !empty($r['tgl_kirim_eff'])
            ? $r['tgl_kirim_eff']
            : ($r['tgl_kirim_srj'] ?? null);
...
// Jika diminta download sebagai Excel/CSV, keluarkan CSV dengan BOM UTF-8
$wantCsv = (isset($_GET['format']) && strtolower($_GET['format']) === 'excel') || (isset($_GET['download']) && $_GET['download'] == '1');
if ($wantCsv) {
    $filename = 'realisasi_' . date('Ymd_His') . '.csv';
    ...
    if (!empty($rows)) {
        // header kolom berdasarkan keys baris pertama
        $first = $rows[0];
        fputcsv($out, array_keys($first));
        foreach ($rows as $r) {
            // ubah value array menjadi JSON string agar tetap masuk satu cell
        ...
        // tulis header kosong jika tidak ada data
        fputcsv($out, ['No data']);
    }
    fclose($out);
    exit;
