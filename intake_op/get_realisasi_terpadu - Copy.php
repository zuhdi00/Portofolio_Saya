...
LEFT JOIN (
    SELECT
        ...
        -- Tonase: jika lVoid tidak aktif, hitung qty * brt; jika lVoid aktif = 0
        SUM(CASE
...
// Filter tanggal kirim: berbasis dTanggal tbSRJ (tgl_kirim_srj_min/max)
// Gunakan MIN(dTanggal) sebagai acuan utama untuk range yang lebih luas dan data lengkap
if (!empty($shipFrom) && !empty($shipTo)) {
    // Rentang penuh: kirim harus masuk antara shipFrom dan shipTo
    $where[] = "(srj_agg.tgl_kirim_srj_min >= ? AND srj_agg.tgl_kirim_srj_min <= ?)";
    $params[] = $shipFrom;
    $params[] = $shipTo . ' 23:59:59';
} elseif (!empty($shipFrom)) {
    $where[] = "srj_agg.tgl_kirim_srj_min >= ?";
    $params[] = $shipFrom;
} elseif (!empty($shipTo)) {
    $where[] = "srj_agg.tgl_kirim_srj_min <= ?";
    $params[] = $shipTo . ' 23:59:59';
