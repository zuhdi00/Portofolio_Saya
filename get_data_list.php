...
try {
    ...
    // Format tanggal ke SQL Server 'Y-m-d H:i:s.000'
    ...
    if (isset($_GET['export']) && $_GET['export'] === 'excel') {
        ...
        echo "No. Order\tSC No.\tMC No.\tClient\tProduct\tTanggal OP\tDimensions\tQuality\tQty Order\tQty Produksi\tNetto\tColor\tShipping Date\tType\tApproved By\tLocations\n";
        foreach ($data as $row) {
            $dimensions = "{$row['nPanjang']}x{$row['nLebar']}x{$row['nTinggi']}";
            $quality = "{$row['ckd_b1']} / {$row['ckd_b2']} / {$row['ckd_b3']} / {$row['ckd_b4']} / {$row['ckd_b5']}";
            $shippingDate = $row['lTK'] == '1' ? 'Tunggu Kabar' : ($row['dTglkirim'] ? substr($row['dTglkirim'], 0, 10) : '');
            echo "{$row['cNoOp']}\t{$row['cNoSc']}\t{$row['cNoMc']}\t{$row['cnm_c']}\t{$row['cnm_brg']}\t" . ($row['dTgl'] ? substr($row['dTgl'], 0, 10) : '') . "\t{$dimensions}\t{$quality}\t{$row['nQty']}\t{$row['nQtyStok']}\t{$row['nTot_netto']}\t{$row['cWarna']}\t{$shippingDate}\t{$row['cTipe']}\t{$row['cMengetahui']}\t" . mapRackCode($row['cRak']) . "\n";
        }
        exit;
    }
