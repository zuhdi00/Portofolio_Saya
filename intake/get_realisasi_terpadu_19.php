...
-- Root dari tbSRJDtl INNER JOIN tbSRJ agar semua baris SRJ pasti muncul.
-- Kemudian LEFT JOIN ke tbOP — OP yang ada di SRJ tapi tidak ada di tbOP tetap tampil.
-- Ini menghindari data hilang saat filter dTanggal/cNoSRJ aktif.
FROM (
    SELECT DISTINCT d.cNoOp
    FROM tbSRJDtl d WITH (NOLOCK)
    INNER JOIN tbSRJ s WITH (NOLOCK) ON s.cNoSRJ = d.cNoSRJ
    WHERE d.cNoOp IS NOT NULL AND d.cNoOp <> ''
) AS srj_base
LEFT JOIN tbOP op WITH (NOLOCK) ON op.cNoOp = srj_base.cNoOp

-- JOIN ke tbSC dari cNoSc di tbOP
...
LEFT JOIN (
    SELECT
        ...
        -- Tonase: jika lVoid tidak aktif, jumlahkan (d.nQty * d.nBrtOp)
        MAX(CASE
...
if (!empty($dateScFrom)){
    // Accept SC date OR OP date to avoid excluding OPs belonging to the SC
    $where[] = "(sc.dTanggal >= ? OR op.dTgl >= ?)";
    $params[] = $dateScFrom;
    $params[] = $dateScFrom;
}
if (!empty($dateScTo)) {
...
// Filter tanggal kirim: gunakan s2.dTanggal dari tbSRJ (via join tbSRJDtl.cNoOp = op.cNoOp)
// Hanya OP yang punya SRJ dengan dTanggal dalam rentang yang ditampilkan.
// OP yang belum punya SRJ sama sekali TIDAK ikut ditampilkan saat filter aktif.
...
// Count query menggunakan root yang sama: tbSRJDtl → tbOP → tbSC
