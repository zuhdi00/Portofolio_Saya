...
// Cek SRJ aktual (sudah terkirim) ATAU sc.dTglKirim2 (jadwal kirim, belum kirim)
// Sehingga OP/SC yang belum ada SRJ-nya tetap muncul jika jadwal kirimnya dalam rentang
if (!empty($shipFrom) && !empty($shipTo)) {
    $where[] = "(
        ...
        OR (
            CAST(sc.dTglKirim2 AS DATE) >= ?
            AND CAST(sc.dTglKirim2 AS DATE) <= ?
        )
    )";
    ...
    $where[] = "(
        ...
        OR CAST(sc.dTglKirim2 AS DATE) >= ?
    )";
    $params[] = $shipFrom; $params[] = $shipFrom;
} elseif (!empty($shipTo)) {
    $where[] = "(
        EXISTS (
            SELECT 1 FROM tbSRJDtl d2 WITH (NOLOCK)
