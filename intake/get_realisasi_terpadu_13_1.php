...
// Allow long-running requests for large exports/queries
@set_time_limit(0);
...
// ══════════════════════════════════════════════════════════════════════════════
// ACTION: srj_per_tgl — laporan SRJ per tanggal, mirip Crystal Report RpTmpSrjPerTgl
// Filter: date_from, date_to (wajib salah satu), sc_no, customer, sales
// Tanggal efektif = COALESCE(tbSRJDtl.UserDate, tbSRJ.dTanggal)
// ══════════════════════════════════════════════════════════════════════════════
if ($action === 'srj_per_tgl') {
    $dateFrom2  = trim($_GET['date_from'] ?? '');
    $dateTo2    = trim($_GET['date_to']   ?? '');
    $scNo2      = trim($_GET['sc_no']     ?? '');
    $customer2  = trim($_GET['customer']  ?? '');
    $sales2     = trim($_GET['sales']     ?? '');

    if (empty($dateFrom2) && empty($dateTo2) && empty($scNo2)) {
    ...
    $srjSql = "SELECT
        -- Identitas SRJ
        s.cNoSRJ,
        s.dTanggal                                          AS tgl_doc_srj,
        d.UserDate                                          AS userdate_dtl,
        -- Tanggal efektif: UserDate jika terisi, fallback ke dTanggal header (sama dg Crystal Report)
        COALESCE(d.UserDate, s.dTanggal)                    AS tgl_efektif,
        s.cTujuanKirim,
        s.cNoPol,
        s.cKeterangan                                       AS ket_srj,

        -- Detail baris
        d.cNoOp,
        d.cNama                                             AS nama_brg,
        d.nQty                                              AS qty_kirim,
        d.nBrtOp                                            AS berat_satuan,
        ISNULL(d.nQty,0) * ISNULL(d.nBrtOp,0)              AS tonase_baris,

        -- SC
        sc.cNoSc,
        sc.cNama                                            AS customer,
        sc.cJenis                                           AS jenis_brg,
        sc.cSales                                           AS sales,

        -- OP
        op.cNoMc,
        op.nQty                                             AS qty_order,
        op.cFlexo,
        op.cJnsGel                                          AS flute,
        op.nPanjang                                         AS p_luar,
        op.nLebar                                           AS l_luar,
        op.nTinggi                                          AS t_luar

    FROM tbSRJDtl d WITH (NOLOCK)
    INNER JOIN tbSRJ s WITH (NOLOCK) ON s.cNoSRJ = d.cNoSRJ
    LEFT JOIN tbOP op WITH (NOLOCK) ON op.cNoOp = d.cNoOp
    ...
    // Filter tanggal efektif (UserDate bila ada, fallback dTanggal)
    if (!empty($dateFrom2) && !empty($dateTo2)) {
        $srjWhere[] = "(
            (d.UserDate IS NOT NULL AND d.UserDate >= ? AND d.UserDate <= ?)
            OR
            (d.UserDate IS NULL AND s.dTanggal >= ? AND s.dTanggal <= ?)
        )";
        $srjParams[] = $dateFrom2;
        $srjParams[] = $dateTo2 . ' 23:59:59';
        $srjParams[] = $dateFrom2;
        $srjParams[] = $dateTo2 . ' 23:59:59';
    } elseif (!empty($dateFrom2)) {
        $srjWhere[] = "(
            (d.UserDate IS NOT NULL AND d.UserDate >= ?)
            OR (d.UserDate IS NULL AND s.dTanggal >= ?)
        )";
        $srjParams[] = $dateFrom2;
        $srjParams[] = $dateFrom2;
    } elseif (!empty($dateTo2)) {
        $srjWhere[] = "(
            (d.UserDate IS NOT NULL AND d.UserDate <= ?)
            OR (d.UserDate IS NULL AND s.dTanggal <= ?)
        )";
    ...
    if (!empty($scNo2))     { $srjWhere[] = "sc.cNoSc LIKE ?";    $srjParams[] = '%'.$scNo2.'%'; }
    if (!empty($customer2)) { $srjWhere[] = "sc.cNama LIKE ?";    $srjParams[] = '%'.$customer2.'%'; }
    if (!empty($sales2))    { $srjWhere[] = "sc.cSales LIKE ?";   $srjParams[] = '%'.$sales2.'%'; }

    if (!empty($srjWhere)) {
        $srjSql .= " AND " . implode(" AND ", $srjWhere);
    }
    $srjSql .= " ORDER BY COALESCE(d.UserDate, s.dTanggal), s.cNoSRJ, d.cNoOp";

    $srjRows = fetchAll($conn, $srjSql, $srjParams);
    if (isset($srjRows['error'])) {
        sqlsrv_close($conn);
        echo json_encode(['success' => false, 'message' => 'Query srj_per_tgl gagal.', 'errors' => $srjRows['error']], JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    $totalQty    = 0;
    $totalTonase = 0;
    foreach ($srjRows as &$row) {
        foreach ($row as $k => $v) {
            if (is_string($v)) $row[$k] = safeStr($v);
        }
        $totalQty    += (float)($row['qty_kirim']   ?? 0);
        $totalTonase += (float)($row['tonase_baris'] ?? 0);
    }
    unset($row);
...
if ($action === 'detail') {
    ...
    // Pengiriman — sertakan UserDate dan dTanggal agar tanggal efektif bisa dihitung di frontend
    $srj = fetchAll($conn,
        "SELECT d.cNoSRJ, d.cNama, d.nQty, d.cNoOp, d.cNoScDtl,
                d.UserDate,
                s.dTanggal, s.cKeterangan, s.cNoPol, s.cTujuanKirim,
                -- Tanggal efektif: UserDate detail jika terisi, fallback ke dTanggal header
                COALESCE(d.UserDate, s.dTanggal) AS tgl_efektif
         FROM tbSRJDtl d WITH (NOLOCK)
         INNER JOIN tbSRJ s WITH (NOLOCK) ON d.cNoSRJ = s.cNoSRJ
         WHERE COALESCE(s.lVoid, '0') <> '1'
           AND (d.cNoScDtl = ? OR d.cNoOp LIKE ? OR s.cNoSC = ?)
         ORDER BY COALESCE(d.UserDate, s.dTanggal), s.cNoSRJ",
        [$sc, $sc.'%', $sc]
    );
...
$sql = "SELECT
    ...
    -- tgl_kirim_srj_min = MIN(COALESCE(UserDate detail, dTanggal header)) → tanggal efektif pertama
    srj_agg.tgl_kirim_srj_min                   AS tgl_kirim_srj_min,
    srj_agg.tgl_kirim_srj_max                   AS tgl_kirim_srj_max,
    -- tgl_doc_srj: tanggal dokumen SRJ (header) murni, tanpa UserDate
    srj_agg.tgl_doc_srj_min                     AS tgl_doc_srj_min,
    srj_agg.tgl_doc_srj_max                     AS tgl_doc_srj_max,
    srj_agg.userdate_srj                        AS userdate_srj,
    srj_agg.tujuan_kirim                        AS tujuan_kirim,
    ISNULL(srj_agg.tonase_rows, 0) AS tonase,
...
-- OP: LEFT JOIN tbOP → jika ada beberapa OP per SC, semua OP akan dikembalikan
...
-- SRJ aggregat: per cNoOp
-- Menggunakan COALESCE(d.UserDate, s.dTanggal) sebagai tanggal efektif pengiriman,
-- sesuai logika Crystal Report TbTmpSrjPerTgl (field dTanggal = header SRJ, UserDate = detail SRJ).
-- Jika UserDate diisi di tbSRJDtl → pakai UserDate sebagai tanggal kirim efektif.
-- Jika UserDate kosong → fallback ke dTanggal header tbSRJ.
LEFT JOIN (
    SELECT
        d.cNoOp                                             AS cNoOp,
        SUM(ISNULL(d.nQty,0))                               AS jml_kirim,
        -- Tanggal efektif pertama: COALESCE(UserDate detail, dTanggal header)
        MIN(COALESCE(d.UserDate, s.dTanggal))               AS tgl_kirim_srj_min,
        -- Tanggal efektif terakhir
        MAX(COALESCE(d.UserDate, s.dTanggal))               AS tgl_kirim_srj_max,
        -- Tanggal dokumen SRJ (header) untuk referensi
        MIN(s.dTanggal)                                     AS tgl_doc_srj_min,
        MAX(s.dTanggal)                                     AS tgl_doc_srj_max,
        -- UserDate MAX dari detail (informasi tambahan)
        MAX(d.UserDate)                                     AS userdate_srj,
        -- Tujuan pengiriman
        MAX(s.cTujuanKirim)                                 AS tujuan_kirim,
        -- Tonase: SUM (bukan MAX) agar akumulasi benar, skip row void
        MAX(CASE
            WHEN COALESCE(s.lVoid, '0') = '1' THEN 0
...
// Filter tanggal kirim efektif:
// Logika SAMA dengan Crystal Report TbTmpSrjPerTgl:
//   - Jika tbSRJDtl.UserDate terisi → tanggal kirim = UserDate (tanggal aktual kirim ke customer)
//   - Jika tbSRJDtl.UserDate kosong → fallback ke tbSRJ.dTanggal (tanggal dokumen SRJ)
// Menggunakan OR untuk menangkap SEMUA kemungkinan:
//   (a) UserDate diisi dan dalam range → masuk
//   (b) UserDate kosong DAN dTanggal dalam range → masuk
// Sehingga tidak ada data yang terlewat akibat inkonsistensi pengisian UserDate.
if (!empty($shipFrom) && !empty($shipTo)) {
    $where[] = "EXISTS (
        ...
        WHERE (
              d2.cNoOp = op.cNoOp
           OR d2.cNoScDtl = sc.cNoSc
           OR s2.cNoSC = sc.cNoSc
        )
          AND COALESCE(s2.lVoid, '0') <> '1'
          AND (
              -- (a) UserDate diisi: gunakan UserDate sebagai tgl kirim efektif
              (d2.UserDate IS NOT NULL
               AND d2.UserDate >= ?
               AND d2.UserDate <= ?)
              OR
              -- (b) UserDate kosong: fallback ke dTanggal header SRJ
              (d2.UserDate IS NULL
               AND s2.dTanggal >= ?
               AND s2.dTanggal <= ?)
          )
    ...
    $where[] = "EXISTS (
        ...
        WHERE (
          ...
          AND COALESCE(s2.lVoid, '0') <> '1'
          AND (
              (d2.UserDate IS NOT NULL AND d2.UserDate >= ?)
              OR
              (d2.UserDate IS NULL AND s2.dTanggal >= ?)
          )
    )";
    $params[] = $shipFrom;
    $params[] = $shipFrom;
...
$countParams = $params;
$cStmt = sqlsrv_query($conn, $countSql, empty($countParams) ? [] : $countParams, ["QueryTimeout"=>3600]);
$total = 0;
if ($cStmt) {
...
$stmt = sqlsrv_query($conn, $sql, empty($params) ? [] : $params, ["QueryTimeout"=>3600]);
if ($stmt === false) {
    $errs = sqlsrv_errors();
    sqlsrv_close($conn);
...
if ($stmt !== false) {
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        ...
        // Tanggal kirim aktual efektif: MIN(COALESCE(UserDate, dTanggal)) → sesuai Crystal Report
        $r['tgl_kirim_aktual'] = !empty($r['tgl_kirim_srj_min'])
            ? $r['tgl_kirim_srj_min']
            : null;
        // Tanggal kirim terakhir efektif
        $r['tgl_kirim_terakhir'] = $r['tgl_kirim_srj_max'] ?? null;
        // Tanggal dokumen SRJ (header, tanpa UserDate) — untuk audit
        $r['tgl_doc_kirim']     = $r['tgl_doc_srj_min'] ?? null;
