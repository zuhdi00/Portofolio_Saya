...
$sql = "SELECT
    ...
    ISNULL(srj_agg.tonase_rows, 0) AS tonase_op,
    ISNULL(srj_agg_sc.tonase_sc, 0) AS tonase_sc,
    -- Daftar nomor SRJ terkait SC (gabungan dari tbSRJ.cNoSRJ dan tbSRJDtl.cNoSRJ yang merujuk ke SC)
    ISNULL(srj_sc_agg.srj_list, '') AS srj_list,
...
LEFT JOIN (
    ...
    LEFT JOIN (
        SELECT d2.cNoOp, MIN(d2.UserDate) AS tgl_first
...
-- SRJ per SC: daftar semua cNoSRJ yang terkait dengan SC (kompatibel SQL Server lama)
LEFT JOIN (
    SELECT t.sc_key,
        STUFF(
            (
                SELECT ',' + s2.cNoSRJ
                FROM tbSRJ s2 WITH (NOLOCK)
                WHERE s2.cNoSC = t.sc_key
                FOR XML PATH(''), TYPE
            ).value('.', 'NVARCHAR(MAX)'), 1, 1, '')
        AS srj_list
    FROM (
        SELECT DISTINCT s.cNoSC AS sc_key
        FROM tbSRJ s WITH (NOLOCK)
        WHERE s.cNoSC IS NOT NULL
        UNION
        SELECT DISTINCT d.cNoScDtl AS sc_key
        FROM tbSRJDtl d WITH (NOLOCK)
        WHERE d.cNoScDtl IS NOT NULL
    ) t
) srj_sc_agg ON srj_sc_agg.sc_key = sc.cNoSc

LEFT JOIN (
    SELECT
        d.cNoOp,
        SUM(ISNULL(d.nQty,0))                           AS jml_kirim,
        MIN(d.UserDate)                                AS tgl_kirim_srj_min,
        MAX(d.UserDate)                                AS tgl_kirim_srj_max,
        MAX(d.UserDate)                                 AS userdate_srj,
        MAX(s.cTujuanKirim)                             AS tujuan_kirim,
        SUM(CASE
            WHEN COALESCE(vw.lVoid, s.lVoid, '0') = '1' THEN 0
    ...
    LEFT JOIN vwSuratJalan vw WITH (NOLOCK) ON vw.cNoSRJ = s.cNoSRJ
        -- NOTE: avoid joining tbSCDtl here to prevent schema mismatch across environments
    GROUP BY d.cNoOp
    ) srj_agg ON srj_agg.cNoOp = op.cNoOp

    -- SRJ aggregat per SC (gunakan d.cNoScDtl jika ada, fallback ke s.cNoSC)
    LEFT JOIN (
        SELECT
            COALESCE(d.cNoScDtl, s.cNoSC) AS sc_key,
            SUM(ISNULL(d.nQty,0)) AS jml_kirim_sc,
            SUM(CASE
                WHEN COALESCE(vw.lVoid, s.lVoid, '0') = '1' THEN 0
                ELSE ISNULL(d.nQty,0) * ISNULL(d.nBrtOp,0)
            END) AS tonase_sc
    ...
    ) srj_agg_sc ON srj_agg_sc.sc_key = sc.cNoSc
...
if (!empty($shipFrom) && !empty($shipTo)) {
        $where[] = "EXISTS (
                ...
                WHERE (
                    d2.cNoScDtl = sc.cNoSc
                    OR s2.cNoSC = sc.cNoSc
                    OR d2.cNoOp IN (SELECT cNoOp FROM tbOP op2 WITH (NOLOCK) WHERE op2.cNoSc = sc.cNoSc)
                )
                    AND d2.UserDate >= ?
                    AND d2.UserDate <= ?
        )";
...
$summary = [
    'tonase_op_total' => array_sum(array_column($rows, 'tonase_op')),
    'tonase_sc_total' => array_sum(array_column($rows, 'tonase_sc')),
    'jml_kirim_total' => array_sum(array_column($rows, 'jml_kirim')),
];
