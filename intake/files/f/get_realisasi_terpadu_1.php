...
if ($action === 'list') {
    ...
    LEFT JOIN (
        /* Try to use cNoOp from the view; if missing, fall back to mapping via tbSRJDtl */
        SELECT COALESCE(d.cNoOp, dtl.cNoOp) AS cNoOp,
        ...
        LEFT JOIN tbSRJDtl dtl WITH (NOLOCK) ON dtl.cNoSRJ = d.cNoSRJ
        GROUP BY COALESCE(d.cNoOp, dtl.cNoOp)
    ) srj ON srj.cNoOp = op.cNoOp

    LEFT JOIN (
        /* Return rows: fall back to mapping via tbSRJDtl if view lacks cNoOp */
        SELECT COALESCE(r.cNoOp, dtl.cNoOp) AS cNoOp,
        ...
        GROUP BY COALESCE(r.cNoOp, dtl.cNoOp)
    ...
    if ($includeTotals) {
        ...
        LEFT JOIN (
            SELECT r.cNoOp, SUM(ISNULL(r.nBerat,0) * ISNULL(r.nQty,0)) AS kg_retur
            FROM vwReturnSrj r WITH (NOLOCK)
            GROUP BY r.cNoOp
        ) r ON r.cNoOp = op.cNoOp
        LEFT JOIN tbSC sc WITH (NOLOCK) ON sc.cNoSc = op.cNoSc
