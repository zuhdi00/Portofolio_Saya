 ...
 *
 * ── PERUBAHAN DI VERSI INI (v2) ─────────────────────────────────────────────
 * Perhitungan TONASE diperbaiki agar akurat dan acuannya disamakan dengan
 * aplikasi VB (Dashboard Data Generator — frmMain.frm, sub GetDailyTonnage /
 * GetSalesTonnage / GetTotalTonnage). Ditemukan 2 masalah di versi lama:
 *
 *   1) Bug agregasi: versi lama memakai MAX(qty*berat) per cNoOp, sehingga
 *      kalau satu OP dikirim lewat lebih dari satu baris/SRJ, hanya nilai
 *      baris TERBESAR yang terhitung — baris lain hilang. Seharusnya SUM().
 *
 *   2) Rumus tonase versi lama tidak menerapkan pengecualian VB:
 *          CASE WHEN cTipe LIKE 'SF%' AND nPanjang <= 0
 *                  THEN nQty                  -- qty dihitung langsung sbg tonase
 *               ELSE nQty * nBrtOp             -- qty x berat per unit (kg)
 *          END
 *      Versi lama selalu pakai (nQty * nBrtOp) tanpa mengecek cTipe/nPanjang,
 *      karena kolom cTipe & nPanjang tidak ikut di-join (lihat komentar lama
 *      "avoid joining tbSCDtl ... schema mismatch"). Akibatnya nilai untuk
 *      tipe SF (mis. sample/flat tanpa panjang) ikut dikali berat → salah.
 *
 * Solusi v2: tarik tonase langsung dari VIEW yang sama dipakai VB
 * (vwSuratJalan + vwSuratJalanDtl, dan vwReturnSrj untuk retur), supaya semua
 * join/derivasi cTipe & nPanjang konsisten dengan sumber aslinya — tidak
 * direkonstruksi manual dari tbSRJDtl/tbSCDtl yang berisiko schema mismatch.
 * Tonase akhir = (SUM tonase kirim − SUM tonase retur) / 1000  →  satuan TON,
 * persis seperti yang ditampilkan VB.
 *
 * ASUMSI YANG PERLU DIVERIFIKASI KE DBA / SKEMA ASLI (silakan koreksi bila
 * nama kolom berbeda di server):
 *   - vwSuratJalanDtl punya kolom: cNoSRJ, cNoOp, cTipe, nPanjang, nQty,
 *     nBrtOp, nTon, UserDate  (cNoOp diasumsikan ikut karena ini kunci utama
 *     yang dipakai di seluruh sistem untuk mengaitkan ke tbOP).
 *   - vwSuratJalan punya kolom: cNoSRJ, dTanggal, cTujuanKirim, lVoid, cSales.
 *   - vwReturnSrj punya kolom: cNoOp, nBerat, nQty (dipakai VB untuk hitung
 *     tonase retur = SUM(nBerat * nQty)).
 * Jika salah satu asumsi di atas tidak sesuai skema, query akan error/0 —
 * laporkan nama kolom sebenarnya agar langsung disesuaikan.
 */
...
if ($action === 'detail') {
    ...
    // Pengiriman — sumber vwSuratJalan/vwSuratJalanDtl (sama dgn VB) supaya
    // kolom cTipe/nPanjang/nBrtOp/nTon yg dipakai utk hitung tonase tersedia.
    $srj = fetchAll($conn,
        "SELECT d.cNoSRJ, d.cNama, d.nQty, d.cNoOp, d.cNoScDtl,
                d.cTipe, d.nPanjang, d.nBrtOp, d.nTon,
                s.dTanggal, s.cKeterangan, s.cNoPol, s.cTujuanKirim, s.lVoid
         FROM vwSuratJalanDtl d
         INNER JOIN vwSuratJalan s ON d.cNoSRJ = s.cNoSRJ
         WHERE d.cNoScDtl = ? OR d.cNoOp LIKE ? OR s.cNoSC = ?
         ORDER BY s.dTanggal",
        [$sc, $sc.'%', $sc]
    );

    // Retur — tambahkan nBerat/nQty (sumber vwReturnSrj) utk hitung tonase retur
    $returTon = fetchAll($conn,
        "SELECT cNoOp, SUM(ISNULL(nBerat,0) * ISNULL(nQty,0)) AS tonase_kg_retur
         FROM vwReturnSrj
         WHERE cNoOp IN (SELECT cNoOp FROM tbOP WHERE cNoSc = ? OR cNoOp LIKE ?)
         GROUP BY cNoOp",
        [$sc, $sc.'%']
    );
    $totalReturTon = array_sum(array_column($returTon, 'tonase_kg_retur'));
    ...
    // ── TONASE (v2, acuan VB) ────────────────────────────────────────────
    // Rumus per baris SRJ: CASE WHEN cTipe LIKE 'SF%' AND nPanjang <= 0
    //                            THEN nQty ELSE nQty * nBrtOp END
    // (identik dgn frmMain.GetDailyTonnage/GetSalesTonnage/GetTotalTonnage),
    // dijumlahkan SELURUH baris (bukan diambil nilai terbesar saja).
    $totalTonaseKgKirim = 0.0;
    foreach ($srj as $r) {
        if ((string)($r['lVoid'] ?? '0') === '1') continue; // baris void, abaikan
        $cTipe    = strtoupper(trim($r['cTipe'] ?? ''));
        $nPanjang = (float)($r['nPanjang'] ?? 0);
        $nQty     = (float)($r['nQty'] ?? 0);
        $nBrtOp   = (float)($r['nBrtOp'] ?? 0);
        if (strpos($cTipe, 'SF') === 0 && $nPanjang <= 0) {
            $totalTonaseKgKirim += $nQty;
        } else {
            $totalTonaseKgKirim += $nQty * $nBrtOp;
        }
    }
    $totalTonaseTonNet = round(($totalTonaseKgKirim - $totalReturTon) / 1000, 3);
    ...
    echo json_encode([
        ...
        'totals' => [
            ...
            // Tonase v2 (kg sebelum dibagi 1000, dan ton net seperti tampilan VB)
            'tonase_kg_kirim' => round($totalTonaseKgKirim, 3),
            'tonase_kg_retur' => round($totalReturTon, 3),
            'tonase_ton_net'  => $totalTonaseTonNet,
        ]
...
$sql = "SELECT
    ...
    -- TONASE (v2, acuan VB — lihat header file):
    ISNULL(srj_agg.tonase_kg_kirim, 0)          AS tonase_kg_kirim,   -- kg, sebelum dikurangi retur
    ISNULL(retur_ton_agg.tonase_kg_retur, 0)    AS tonase_kg_retur,   -- kg, retur
    ISNULL(srj_agg.tonase2_rows, 0)             AS tonase2,           -- legacy/cross-check (kolom nTon)
...
-- tgl_kirim_srj_min = dTanggal SRJ pertama (acuan filter tanggal kirim utama)
-- tgl_kirim_srj_max = dTanggal SRJ terakhir (untuk tracking)
-- userdate_srj      = MAX(UserDate) jika diisi (untuk informasi tambahan)
--
-- TONASE (v2 — acuan VB frmMain.GetDailyTonnage/GetSalesTonnage/GetTotalTonnage):
-- Sumber: vwSuratJalan + vwSuratJalanDtl (view yang sama dipakai VB), BUKAN
-- direkonstruksi manual dari tbSRJDtl/tbSCDtl, agar kolom cTipe & nPanjang
-- (untuk pengecualian SF) konsisten dgn aplikasi VB.
-- Rumus per baris   : CASE WHEN cTipe LIKE 'SF%' AND nPanjang <= 0 THEN nQty
--                           ELSE nQty * nBrtOp END
-- Agregasi per cNoOp: SUM() seluruh baris (BUKAN MAX — itu bug versi lama
-- yang membuang baris selain nilai terbesar saat satu OP dikirim >1 baris).
LEFT JOIN (
    SELECT
        d.cNoOp                                         AS cNoOp,
        SUM(ISNULL(d.nQty,0))                           AS jml_kirim,
        -- Tanggal dokumen SRJ pertama (acuan utama filter)
        MIN(s.dTanggal)                                 AS tgl_kirim_srj_min,
        -- Tanggal dokumen SRJ terakhir (untuk tracking)
        MAX(s.dTanggal)                                 AS tgl_kirim_srj_max,
        -- UserDate per baris detail (informasi tambahan)
        MAX(d.UserDate)                                 AS userdate_srj,
        -- Tujuan pengiriman
        MAX(s.cTujuanKirim)                             AS tujuan_kirim,
        -- Tonase KIRIM (kg, sebelum dikurangi retur) — rumus identik VB,
        -- di-SUM per cNoOp (perbaikan dari MAX di versi lama)
        SUM(
            CASE
                WHEN ISNULL(s.lVoid,0) = 1 THEN 0
                WHEN d.cTipe LIKE 'SF%' AND ISNULL(d.nPanjang,0) <= 0
                    THEN ISNULL(d.nQty,0)
                ELSE ISNULL(d.nQty,0) * ISNULL(d.nBrtOp,0)
            END
        ) AS tonase_kg_kirim,
        -- Referensi/legacy: kolom nTon yang sudah tersimpan di transaksi
        -- (dipakai utk audit/cross-check terhadap hasil hitung di atas)
        SUM(ISNULL(d.nTon, 0))                          AS tonase2_rows
    FROM vwSuratJalanDtl d WITH (NOLOCK)
    INNER JOIN vwSuratJalan s WITH (NOLOCK) ON s.cNoSRJ = d.cNoSRJ
    GROUP BY d.cNoOp
) srj_agg ON srj_agg.cNoOp = op.cNoOp

-- Tonase RETUR (kg) — acuan VB: SUM(nBerat * nQty) dari vwReturnSrj,
-- dikaitkan ke cNoOp agar bisa dikurangkan dari tonase kirim (tonase NET).
LEFT JOIN (
    SELECT
        cNoOp                                           AS cNoOp,
        SUM(ISNULL(nBerat,0) * ISNULL(nQty,0))          AS tonase_kg_retur
    FROM vwReturnSrj WITH (NOLOCK)
    GROUP BY cNoOp
) retur_ton_agg ON retur_ton_agg.cNoOp = op.cNoOp
...
// Count query — mirror main query filters using same root (tbSC LEFT JOIN tbOP)
...
if ($stmt !== false) {
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        ...
        // ── TONASE (v2, acuan VB) ───────────────────────────────────────
        // tonase_kg_kirim & tonase_kg_retur sudah dihitung akurat di SQL
        // (SUM per baris + pengecualian cTipe SF, bukan lagi MAX yg salah).
        // tonase_ton_net = (kirim - retur) / 1000, persis seperti yg
        // ditampilkan VB (VB juga membagi 1000 sebelum ditampilkan).
        $tonaseKgKirim       = (float)($r['tonase_kg_kirim'] ?? 0);
        $tonaseKgRetur       = (float)($r['tonase_kg_retur'] ?? 0);
        $r['tonase_kg_kirim'] = $tonaseKgKirim;
        $r['tonase_kg_retur'] = $tonaseKgRetur;
        $r['tonase_ton_net']  = round(($tonaseKgKirim - $tonaseKgRetur) / 1000, 3);
        // tonase2 (legacy, dari kolom nTon transaksi) dibiarkan mentah utk audit/banding
