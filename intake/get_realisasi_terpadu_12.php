...
// Simple request/error logging for debugging 500/503 on hosting
$debugLog = __DIR__ . '/realisasi_error.log';
...
app_log("REQUEST: {$_SERVER['REMOTE_ADDR']} {$_SERVER['REQUEST_METHOD']} {$_SERVER['REQUEST_URI']} QS=" . ($_SERVER['QUERY_STRING'] ?? ''));

// Capture PHP errors and fatal shutdown for diagnostics
set_error_handler(function($errno, $errstr, $errfile, $errline) {
    app_log("PHP_ERROR [$errno] $errstr in $errfile:$errline");
    return false; // continue normal error handling
});
register_shutdown_function(function() {
    $err = error_get_last();
    if ($err) {
        app_log("SHUTDOWN_ERROR [{$err['type']}] {$err['message']} in {$err['file']}:{$err['line']}");
    }
});

// Check required extension early
if (!extension_loaded('sqlsrv')) {
    app_log('MISSING_EXTENSION: sqlsrv not loaded');
    http_response_code(500);
    echo json_encode(['success'=>false,'message'=>'Server misconfigured: sqlsrv extension missing.']);
    exit;
}
