...
// --- Error logging for hosting diagnostics ---------------------------------
$errLog = __DIR__ . '/realisasi_error.log';
function log_error_msg($msg) {
    global $errLog;
    $line = date('Y-m-d H:i:s') . ' ' . $msg . PHP_EOL;
    @file_put_contents($errLog, $line, FILE_APPEND | LOCK_EX);
}

set_error_handler(function($errno, $errstr, $errfile, $errline){
    $txt = "PHP ERROR [$errno] $errstr in $errfile:$errline";
    log_error_msg($txt);
    return false; // continue with normal error handler
});
set_exception_handler(function($ex){
    $txt = "Uncaught Exception: " . $ex->getMessage() . " in " . $ex->getFile() . ':' . $ex->getLine();
    log_error_msg($txt);
    http_response_code(500);
    echo json_encode(['success'=>false,'message'=>'Internal server error.','error'=>substr($ex->getMessage(),0,200)]);
});
register_shutdown_function(function(){
    $err = error_get_last();
    if ($err && ($err['type'] & (E_ERROR | E_PARSE | E_CORE_ERROR | E_COMPILE_ERROR))) {
        $txt = "Fatal PHP shutdown: " . $err['message'] . " in " . $err['file'] . ':' . $err['line'];
        log_error_msg($txt);
        ...
        echo json_encode(['success'=>false,'message'=>'Fatal server error.','error'=>substr($err['message'],0,200)]);
    }
});
...
function dbConnect($serverName, $opts) {
    ...
    if (!$conn) {
        $errs = sqlsrv_errors();
        $errText = is_array($errs) ? json_encode($errs) : print_r($errs, true);
        log_error_msg("DB connect failed: " . $errText);
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Koneksi DB gagal. Lihat log server.', 'errors' => $errs]);
        exit;
    }
    return $conn;
}
...
$noAnyFilter = empty($search) && empty($mc) && empty($client) && empty($product)
            ...
            && empty($dateScFrom) && empty($dateScTo);
if ($noAnyFilter) {
    // Default filter: Tgl SC hari ini
...
-- SRJ aggregat: langsung hitung per-detail nQty * nBrtOp lalu jumlahkan per OP
LEFT JOIN (
    SELECT
        MAX(d.cNoOp)    AS cNoOp,
        ...
        SUM(CASE WHEN COALESCE(vw.lVoid, s.lVoid, '0') = '1' THEN 0 ELSE ISNULL(d.nQty,0) * ISNULL(d.nBrtOp,0) END) AS tonase_rows
    ...
    LEFT JOIN tbSC sc2 WITH (NOLOCK) ON sc2.cNoSc = op2.cNoSc
...
// Filter by SRJ send date (tbSRJ.dTanggal) OR SRJ detail UserDate (tbSRJDtl.UserDate)
// Include any SRJ related to the SC/OP via d.cNoOp, d.cNoScDtl, or s.cNoSC
if (!empty($shipFrom) && !empty($shipTo)) {
    $where[] = "EXISTS (SELECT 1 FROM tbSRJ s INNER JOIN tbSRJDtl d ON s.cNoSRJ = d.cNoSRJ WHERE (d.cNoOp = op.cNoOp OR d.cNoScDtl = sc.cNoSc OR s.cNoSC = sc.cNoSc) AND ((s.dTanggal >= ? AND s.dTanggal <= ?) OR (d.UserDate >= ? AND d.UserDate <= ?)) )";
    $params[] = $shipFrom;
    $params[] = $shipTo . ' 23:59:59';
    // duplicated for d.UserDate placeholders
    $params[] = $shipFrom;
    $params[] = $shipTo . ' 23:59:59';
} else if (!empty($shipFrom)) {
    $where[] = "EXISTS (SELECT 1 FROM tbSRJ s INNER JOIN tbSRJDtl d ON s.cNoSRJ = d.cNoSRJ WHERE (d.cNoOp = op.cNoOp OR d.cNoScDtl = sc.cNoSc OR s.cNoSC = sc.cNoSc) AND (s.dTanggal >= ? OR d.UserDate >= ?) )";
    $params[] = $shipFrom;
    $params[] = $shipFrom;
} else if (!empty($shipTo)) {
    $where[] = "EXISTS (SELECT 1 FROM tbSRJ s INNER JOIN tbSRJDtl d ON s.cNoSRJ = d.cNoSRJ WHERE (d.cNoOp = op.cNoOp OR d.cNoScDtl = sc.cNoSc OR s.cNoSC = sc.cNoSc) AND (s.dTanggal <= ? OR d.UserDate <= ?) )";
    $params[] = $shipTo . ' 23:59:59';
    $params[] = $shipTo . ' 23:59:59';
}
