...
try {
    ...
    // --- Sales achievement: monthly tonase per sales vs target ---
    $salesAchievement = [];
    // Calculate monthly tonase from tbRtSrjDtl (nQty * nBerat) and get sales name from tbSales.cNama
    $salesSql = "SELECT rs.cSales as sales_code, s.cNama as sales_name, s.nTarget as target,
                 CAST(ISNULL(SUM(rtd.nQty * rtd.nBerat),0)/100.0 AS DECIMAL(10,2)) as monthly_ton
                 FROM tbRtSrjDtl rtd
                 INNER JOIN tbRtSrj rs ON rtd.cNomer = rs.cNomer
                 INNER JOIN tbSales s ON rs.cSales = s.cKode AND s.lOut = 0
                 WHERE rs.dTgl >= DATEADD(MONTH, -1, GETDATE())
                 GROUP BY rs.cSales, s.cNama, s.nTarget
                 ORDER BY rs.cSales";

    $salesStmt = sqlsrv_query($conn, $salesSql);
    if ($salesStmt !== false) {
        while ($r = sqlsrv_fetch_array($salesStmt, SQLSRV_FETCH_ASSOC)) {
            $monthlyTon = isset($r['monthly_ton']) ? (float)$r['monthly_ton'] : 0.0;
            $targetVal = isset($r['target']) ? (float)$r['target'] : 0.0;
            $achievement = null;
            if ($targetVal > 0) {
                $achievement = round(($monthlyTon / $targetVal) * 100, 2);
            }

            // Prefer showing sales name; fallback to code
            $salesLabel = isset($r['sales_name']) && trim($r['sales_name']) !== '' ? $r['sales_name'] : ($r['sales_code'] ?? '-');

            $salesAchievement[] = [
                'sales_code' => $r['sales_code'] ?? null,
                'sales' => $salesLabel,
                'date' => date('Y-m-d'),
                'target' => $targetVal > 0 ? number_format($targetVal, 2, '.', '') : null,
                'tonase' => number_format($monthlyTon, 2, '.', '') . ' Ton',
                'achievement_pct' => $achievement !== null ? $achievement . '%' : null
            ];
        }
        sqlsrv_free_stmt($salesStmt);
    } else {
        // on query failure keep empty and optionally log errors
        $salesAchievement = [];
    }
