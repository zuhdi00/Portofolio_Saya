...
function handleApiRequest($conn, $sql, $defaultResponse = ['totalHasil' => 0, 'totalRusak' => 0]) {
    ...
    // Check if the order is valid (exists in tbOP and its cNoSc exists in tbSC)
    $checkSql = "SELECT 1 FROM tbOP op WHERE op.cNoOp = ? AND EXISTS (SELECT 1 FROM tbSC sc WHERE sc.cNoSC = op.cNoSc)";
    $checkResult = executeQuery($conn, $checkSql, [$noOp]);
    ...
    case '/api/hasil-all':
        ...
        if (!$checkResult['success'] || empty($checkResult['data'])) {
            echo json_encode([
                ...
                'totalHasilUpDown' => 0,
                'totalRusakUpDown' => 0,
                'totalHasilDieCutFlat' => 0,
                'totalRusakDieCutFlat' => 0,
                'cFlexo' => ''
            ]);
            break;
        }
