...
try {
    ...
    $sqlTinggi = "
        SELECT
            op.nTinggi AS search_key,
            'tinggi' AS search_type,
            COUNT(*) AS usage_count,
            MAX(op.dTgl) AS last_used,
            MIN(op.dTgl) AS first_used,
            COUNT(DISTINCT op.cnm_c) AS unique_clients,
            SUM(op.nQty) AS total_quantity
        FROM tbOP op
        WHERE op.nTinggi IS NOT NULL AND CAST(op.nTinggi AS VARCHAR) LIKE ?
        GROUP BY op.nTinggi
        ORDER BY usage_count DESC
    ";
    $paramsTinggi = ["%$search%"];
    $stmtTinggi = sqlsrv_prepare($conn, $sqlTinggi, $paramsTinggi);
    if (!$stmtTinggi || !sqlsrv_execute($stmtTinggi)) {
        throw new Exception("Gagal menjalankan query Tinggi: " . getConnectionError(sqlsrv_errors()));
    }
    while ($row = sqlsrv_fetch_array($stmtTinggi, SQLSRV_FETCH_ASSOC)) {
        $formattedRow = [
            'search_key' => trim($row['search_key']),
            'search_type' => $row['search_type'],
            'display_text' => trim($row['search_key']) . ' (Tinggi)',
            'usage_count' => intval($row['usage_count']),
            'unique_clients' => intval($row['unique_clients']),
            'total_quantity' => floatval($row['total_quantity']),
            'last_used' => $row['last_used'] instanceof DateTime ? $row['last_used']->format("Y-m-d H:i:s") : null,
    ...
    echo json_encode([
        'success' => false,
        'message' => "Tidak ditemukan data yang cocok dengan '$search'",
        'search_term' => $search,
        'timestamp' => date('Y-m-d H:i:s')
    ], JSON_UNESCAPED_UNICODE);
