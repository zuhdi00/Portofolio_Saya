...
header('Access-Control-Allow-Headers: Content-Type, X-Requested-With, Authorization, Origin, Accept, Access-Control-Request-Method, Access-Control-Request-Headers');
...
try {
    ...
    $sql = "SELECT TOP ({$searchParams['limit']})
            ...
            WHERE op.cNoMc IS NOT NULL 
                AND op.cNoMc != '' 
                AND op.cNoMc LIKE ?
                AND EXISTS (SELECT 1 FROM tbSC sc WHERE sc.cNoSC = op.cNoSc)
            GROUP BY op.cNoMc
    ...
    $totalMcSql = "SELECT COUNT(DISTINCT cNoMc) as total_mc FROM tbOP op WHERE cNoMc IS NOT NULL AND cNoMc != '' AND EXISTS (SELECT 1 FROM tbSC sc WHERE sc.cNoSC = op.cNoSc)";
