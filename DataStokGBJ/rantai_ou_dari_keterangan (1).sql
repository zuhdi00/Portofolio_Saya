...
/* tbSC BISA punya lebih dari satu baris untuk satu cNoSc. Kalau tidak
   disaring, satu NO. SC menghasilkan beberapa induk dan menabrak primary key.
   Diambil satu saja, yaitu baris yang paling akhir dibuat. */
SELECT sc, induk
INTO   #ou
FROM (
    SELECT RTRIM(cNoSC) AS sc,
           ...
           ROW_NUMBER() OVER (PARTITION BY RTRIM(cNoSC)
                              ORDER BY dTanggal DESC, UserDate DESC) AS rn
    FROM   dbo.tbSC
    WHERE  cKeterangan LIKE 'OU SLC/%'
) z
WHERE  rn = 1
  AND  induk LIKE 'SLC/%'
  AND  LEN(induk) BETWEEN 10 AND 20      -- nomor lama lebih pendek, contoh SLC/0501/0023
  AND  induk <> sc;

CREATE UNIQUE CLUSTERED INDEX IX_ou ON #ou (sc);

-- Berapa NO. SC yang punya baris ganda di tbSC, sekadar catatan
SELECT COUNT(*) AS sc_punya_baris_ganda FROM (
    SELECT RTRIM(cNoSC) AS sc FROM dbo.tbSC
    GROUP BY RTRIM(cNoSC) HAVING COUNT(*) > 1) d;
