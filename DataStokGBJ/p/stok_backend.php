...
if ($isTulis) {
    ...
    try {
        ...
        if ($st === false) {
            $e = sqlsrv_errors();
            $msg = $e ? $e[0]['message'] : 'Perintah gagal';
            // pesan dari THROW di stored procedure sudah ramah, tampilkan apa adanya
            logMsg('TULIS GAGAL (' . $action . '): ' . $msg);
            sqlsrv_close($conn);
            sendJson(['success' => false, 'message' => $msg], 400);
        }
...
// ============================================================================
$tipeSc = []; $perTipe = []; $adaTipe = false; $posisiTipe = null;
