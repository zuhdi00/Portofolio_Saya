...
define('VERSI_BACKEND',   '3.2-batch');
...
if ($action === 'versi') {
    ...
    sendJson([
        ...
        'endpoint' => ['dashboard','refresh','ping','versi','master','riwayat','batch',
...
// ============================================================================
// DAFTAR BATCH STB UNTUK SATU NOMOR OP
// Dipakai panel koreksi supaya bisa memilih batch mana yang dikoreksi.
// ============================================================================
if ($action === 'batch') {
    $sc = inp('sc', '');
    ...
    $ada = false;
    $c = @sqlsrv_query($conn, "SELECT CASE WHEN OBJECT_ID('dbo.vwStokPerStb') IS NULL THEN 0 ELSE 1 END AS ada",
                       [], ["QueryTimeout" => 15]);
    if ($c !== false) { $r = sqlsrv_fetch_array($c, SQLSRV_FETCH_ASSOC); $ada = !empty($r['ada']); sqlsrv_free_stmt($c); }
    if (!$ada) {
        sqlsrv_close($conn);
        sendJson(['success' => true, 'batch' => [], 'siap' => false,
                  'hint' => 'Jalankan koreksi_per_stb.sql supaya koreksi bisa menunjuk batch STB.']);
    }

    // View bisa versi lama (kolom cRak, tanpa cKelompok) atau versi baru
    // (cProses + cKelompok). Kolomnya dideteksi dulu supaya tidak error kalau
    // kelompok_dari_batch.sql belum dijalankan.
    $adaKel = false; $kolProses = 'cRak';
    $ck = @sqlsrv_query($conn,
        "SELECT SUM(CASE WHEN COLUMN_NAME = 'cKelompok' THEN 1 ELSE 0 END) AS ada_kel,
                SUM(CASE WHEN COLUMN_NAME = 'cProses'   THEN 1 ELSE 0 END) AS ada_proses
         FROM   INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'vwStokPerStb'",
        [], ["QueryTimeout" => 15]);
    if ($ck !== false) {
        $r = sqlsrv_fetch_array($ck, SQLSRV_FETCH_ASSOC);
        $adaKel = !empty($r['ada_kel']);
        if (!empty($r['ada_proses'])) $kolProses = 'cProses';
        sqlsrv_free_stmt($ck);
    }

    $batch = [];
    $sqlB = "SELECT cNoSTB, CONVERT(VARCHAR(10), dTglStb, 23) AS dTglStb, cNoOp,
                    ISNULL($kolProses,'') AS cProses, "
          . ($adaKel ? "ISNULL(cKelompok,'LAIN')" : "''") . " AS cKelompok,
                    nQtyMasuk, nQtyKoreksi, nSisaBatch, nUmur
             FROM   dbo.vwStokPerStb WHERE cNoSc = ? ORDER BY dTglStb DESC, cNoSTB DESC";
    $st = @sqlsrv_query($conn, $sqlB, [$sc], ["QueryTimeout" => 60]);
    if ($st !== false) {
        while ($r = sqlsrv_fetch_array($st, SQLSRV_FETCH_ASSOC)) {
            $batch[] = ['stb' => trim($r['cNoSTB']), 'tgl' => $r['dTglStb'],
                        'op' => trim($r['cNoOp'] ?? ''), 'proses' => trim($r['cProses'] ?? ''),
                        'kelompok' => trim($r['cKelompok'] ?? ''),
                        'masuk' => (int)$r['nQtyMasuk'], 'koreksi' => (int)$r['nQtyKoreksi'],
                        'sisa' => (int)$r['nSisaBatch'], 'umur' => (int)$r['nUmur']];
        }
        sqlsrv_free_stmt($st);
    }
    sqlsrv_close($conn);
    sendJson(['success' => true, 'sc' => $sc, 'batch' => $batch, 'siap' => true,
              'kelompok_otomatis' => $adaKel,
              'hint' => $adaKel ? null
                        : 'Kelompok tipe belum bisa ditentukan otomatis dari batch. Jalankan kelompok_dari_batch.sql supaya tidak perlu dipilih manual.']);
}
...
if ($action === 'riwayat') {
    ...
    $q = @sqlsrv_query($conn,
        "SELECT nId, CONVERT(VARCHAR(10), dTanggal, 23) AS dTanggal, nQtyPc, cJenis,
                ...
                ISNULL(cKelompok,'') AS cKelompok,
                ISNULL(CASE WHEN COL_LENGTH('dbo.tbStokGudangKoreksi','cNoSTB') IS NULL
                            THEN '' ELSE cNoSTB END, '') AS cNoSTB
    ...
    if ($q !== false) {
        while ($r = sqlsrv_fetch_array($q, SQLSRV_FETCH_ASSOC)) {
            $koreksi[] = ['id' => (int)$r['nId'], 'tanggal' => $r['dTanggal'], 'qty' => (int)$r['nQtyPc'],
                          ...
                          'kelompok' => trim($r['cKelompok'] ?? ''), 'stb' => trim($r['cNoSTB'] ?? '')];
        }
        sqlsrv_free_stmt($q);
    }
    ...
    try {
        if ($action === 'simpan_koreksi') {
            ...
            $par = [$sc, $qty, inp('jenis', ''), inp('keterangan', ''), $user,
                    ...
                    inp('kelompok') ?: null, inp('stb') ?: null];
