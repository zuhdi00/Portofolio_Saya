...
";
    ...
    if ($stmt === false) {
        $err = sqlsrv_errors();
        sqlsrv_close($conn);
        sendJson(['success' => false, 'message' => $err ? $err[0]['message'] : 'Query gagal'], 500);
    }
    ...
    logMsg("SLC lookup '{$slcSearch}' rows={$count} duration={$dur}");
...
$dur = microtime(true) - $qstart;
logMsg("Date-range report start={$start} end={$end} status={$statusFilt} rows={$count} duration={$dur}");
