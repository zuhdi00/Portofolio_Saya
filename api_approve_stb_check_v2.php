...
$sqlGetOp = "SELECT cNoOp FROM tbOp WHERE cNoOp = ?";
...
// Hitung total nQty untuk OP yang sama di tbWIPV2
$sqlTotalQty = "SELECT SUM(nQty) as totalQty FROM tbWIPV2 WHERE cNoOp = ?";
...
if ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
    ...
    if ($stmtInsert) {
        // Update lPosted dan cKeterangan di tbOp
        $sqlUpdate = "UPDATE tbOp SET lPosted = 1, cKeterangan = ? WHERE cNoOp = ?";
