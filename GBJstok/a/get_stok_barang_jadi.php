...
// ============================================================
// CACHE SEDERHANA (file-based)
// Tujuan: mempercepat tarikan berulang untuk rentang tanggal yang
// sama (mis. user refresh / pindah tab lalu balik lagi) tanpa
// harus query ulang ke SQL Server setiap saat.
// - Cache key  = kombinasi tgl_mulai + tgl_akhir
// - Cache TTL  = 5 menit (cukup untuk laporan stok yang tidak
//   berubah tiap detik). Ubah $cacheTtl bila perlu lebih real-time.
// - Force refresh: tambahkan &nocache=1 di URL.
// ============================================================
...
$cacheFile = $cacheDir . '/stokbj_' . md5($dtStart . '|' . $dtEnd) . '.json';

if (!$noCache && is_file($cacheFile) && (time() - filemtime($cacheFile)) < $cacheTtl) {
    sqlsrv_close($conn); // koneksi DB belum dipakai, langsung tutup
...
$payloadJson = safeJsonEncode($payload);

// Simpan ke cache untuk request berikutnya dengan rentang tanggal yang sama
@file_put_contents($cacheFile, $payloadJson, LOCK_EX);
