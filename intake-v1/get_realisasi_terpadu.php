...
if (!empty($shipFrom))  { $where[] = "op.dTglkirim >= ?";     $params[] = $shipFrom; }
if (!empty($shipTo))    { $where[] = "op.dTglkirim <= ?";     $params[] = $shipTo.' 23:59:59'; }
if (!empty($dt))        { $where[] = "op.dTglkirim >= ?";     $params[] = $dt; $where[] = "op.dTglkirim <= ?"; $params[] = $dt.' 23:59:59'; }
...
$maxExportRows = 0; // safety cap for exports (0 = no cap)
$scList = [];
...
// For the SC preselect query only apply WHERE clauses that reference SC or safe OP fields.
// Exclude clauses that reference OP date columns which may not exist in some schemas (dTgl, dTglkirim, etc.).
$scWhere = [];
$scParams = [];
foreach ($where as $idx => $clause) {
    if (preg_match('/op\.dTglkirim|op\.dTglkirim2|op\.dTgl/i', $clause)) continue;
    $scWhere[] = $clause;
    if (isset($params[$idx])) $scParams[] = $params[$idx];
}
if (!empty($scWhere)) $scSelect .= " AND " . implode(" AND ", $scWhere);

$scStmt = sqlsrv_query($conn, $scSelect, empty($scParams) ? [] : $scParams, ["QueryTimeout" => 120]);
if ($scStmt === false) {
    $err = sqlsrv_errors();
...
if ($maxExportRows > 0 && count($scList) > $maxExportRows) {
    sqlsrv_close($conn);
...
LEFT JOIN (
    SELECT cNoSc, MIN(cNoOp) AS cNoOp, MIN(cNoMc) AS cNoMc,
           MIN(dTgl) AS dTgl, MIN(dTglkirim) AS dTglkirim,
...
if ($stmt !== false) {
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        ...
        // Ensure tonase numeric
        $r['tonase'] = (float)($r['tonase'] ?? 0);
