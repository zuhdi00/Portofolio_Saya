...
// CATATAN: Tidak ada default date range — tampilkan semua data.
// Filter tanggal hanya aktif jika user mengisi input date_from / date_to.

// --- Main OP query (join stb & srj) ─────────────────────────────────────────
$sql = "SELECT
    ...
    -- Stock Awal Gudang (sebelum pengiriman)
    ISNULL((SELECT SUM(ISNULL(s.nQty,0)) FROM tbStbBJ s WHERE s.cNoOp = op.cNoOp AND s.dTglSerah < ISNULL((SELECT MIN(s2.dTanggal) FROM tbSRJ s2 INNER JOIN tbSRJDtl d2 ON s2.cNoSRJ = d2.cNoSRJ WHERE d2.cNoOp = op.cNoOp), GETDATE())), 0) AS stock_awal_gudang,
    -- Stock Akhir Gudang (setelah pengiriman terakhir)
    ISNULL((SELECT SUM(ISNULL(s.nQty,0)) FROM tbStbBJ s WHERE s.cNoOp = op.cNoOp AND s.dTglSerah <= ISNULL((SELECT MAX(s2.dTanggal) FROM tbSRJ s2 INNER JOIN tbSRJDtl d2 ON s2.cNoSRJ = d2.cNoSRJ WHERE d2.cNoOp = op.cNoOp), GETDATE())), 0) AS stock_akhir_gudang,

        -- Serah Terima (total nQty dari tbStbBJ per OP)
        ISNULL(ISNULL(stb_agg.jml_stb, (SELECT SUM(ISNULL(s.nQty,0)) FROM tbStbBJ s WHERE s.cNoOp = op.cNoOp)), 0) AS jml_serah_trm,
        ISNULL(stb_agg.tgl_serah, (SELECT MAX(s.dTglSerah) FROM tbStbBJ s WHERE s.cNoOp = op.cNoOp)) AS tgl_serah,
        ISNULL(stb_agg.cRak, (SELECT MAX(s.cRak) FROM tbStbBJ s WHERE s.cNoOp = op.cNoOp)) AS cRak,
        ISNULL(stb_agg.cShift, (SELECT MAX(s.cShift) FROM tbStbBJ s WHERE s.cNoOp = op.cNoOp)) AS cShift,

        -- Pengiriman (total nQty dari tbSRJDtl per OP)
        ISNULL(ISNULL(srj_agg.jml_kirim, (SELECT SUM(ISNULL(d.nQty,0)) FROM tbSRJDtl d WHERE d.cNoOp = op.cNoOp)), 0) AS jml_kirim,
        ISNULL(srj_agg.tgl_kirim_srj, (SELECT MAX(s.dTanggal) FROM tbSRJ s INNER JOIN tbSRJDtl d ON d.cNoSRJ = s.cNoSRJ WHERE d.cNoOp = op.cNoOp)) AS tgl_kirim_srj,
        ISNULL(srj_agg.tujuan_kirim, (SELECT MAX(s.cTujuanKirim) FROM tbSRJ s INNER JOIN tbSRJDtl d ON d.cNoSRJ = s.cNoSRJ WHERE d.cNoOp = op.cNoOp)) AS tujuan_kirim,

        -- Corr Hasil (per OP): use direct per-OP scalar subqueries to ensure parity
        ISNULL((SELECT SUM(ISNULL(d.nHasil,0)) FROM tbHslCorrDtl d WHERE d.cNoOp = op.cNoOp), 0) AS hsl_corr,
        ISNULL((SELECT SUM(ISNULL(d.nRusak,0)) FROM tbHslCorrDtl d WHERE d.cNoOp = op.cNoOp), 0) AS rsak_corr,
        ISNULL((SELECT SUM(ISNULL(d.nBerat,0)) FROM tbHslCorrDtl d WHERE d.cNoOp = op.cNoOp), 0) AS berat_corr,
        ISNULL((SELECT SUM(ISNULL(cd.nQtyOrder,0)) FROM tbCorrDtl cd WHERE cd.cNoOp = op.cNoOp), 0) AS plan_corr,

        -- Conv Hasil (ambil dari tbOP.nQtyStok per OP) dan rsak_conv dari tbConvPlanDtl
        ISNULL(op.nQtyStok, 0) AS hsl_conv,
        ISNULL(ISNULL(conv_agg.rsak_conv, (SELECT SUM(ISNULL(d.nRusak,0)) FROM tbConvPlanDtl d WHERE d.cNoOp = op.cNoOp)), 0) AS rsak_conv,

        -- Retur (total dari tbRtSrjDtl via tbRtSrj -> tbSRJDtl -> cNoOp)
        ISNULL(ISNULL(retur_agg.jml_retur, (SELECT SUM(ISNULL(rd.nQty,0))
            FROM tbRtSrjDtl rd
            INNER JOIN tbRtSrj r ON rd.cNomer = r.cNomer
            INNER JOIN tbSRJDtl d2 ON d2.cNoSRJ = r.cNoSrj
            WHERE d2.cNoOp = op.cNoOp)), 0) AS jml_retur

FROM tbOP op

LEFT JOIN tbSC sc ON sc.cNoSc = op.cNoSc

-- STB aggregat
LEFT JOIN (
    SELECT cNoOp,
           SUM(ISNULL(nQty,0)) AS jml_stb,
           MAX(dTglSerah)       AS tgl_serah,
           MAX(cRak)            AS cRak,
           MAX(cShift)          AS cShift
    FROM tbStbBJ
    GROUP BY cNoOp
...
LEFT JOIN (
    SELECT t.cNoOp,
           SUM(ISNULL(t.hsl,0))    AS hsl_corr,
           SUM(ISNULL(t.rusak,0))  AS rsak_corr,
           SUM(ISNULL(t.berat,0))  AS berat_corr,
           SUM(ISNULL(t.plan_qty,0))   AS plan_corr
    FROM (
        SELECT d.cNoOp,
        ...
        SELECT cd.cNoOp,
               ...
               0 AS rusak,
               0 AS berat,
               SUM(ISNULL(cd.nQtyOrder,0)) AS plan_qty
        FROM tbCorrDtl cd
        GROUP BY cd.cNoOp
    ) t
    GROUP BY t.cNoOp
...
-- Conv Hasil aggregat (semua mesin)
LEFT JOIN (
    SELECT d.cNoOp,
           SUM(ISNULL(d.nRusak,0)) AS rsak_conv
    FROM tbConvPlanDtl d
    GROUP BY d.cNoOp
