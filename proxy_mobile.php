...
// Contoh pemakaian:
//   GET  https://supracor.co.id/proxy_mobile.php?path=api_get_item_detail_v2.php&barcode=123
//   POST https://supracor.co.id/proxy_mobile.php?path=api_update_qty.php

// Ambil IP publik dari file log yang diupdate oleh proxi_mobile.php
$logFile = __DIR__ . '/ip_log.txt';
$defaultIP = '36.76.113.224'; // fallback jika log tidak ditemukan
...
if (file_exists($logFile)) {
    ...
    if ($lines && count($lines) > 0) {
        ...
        // Format: [timestamp] Received IP: x.x.x.x | Client: ...
        if (preg_match('/Received IP:\s*([0-9\.]+)/', $lastLine, $matches)) {
            $internalIP = $matches[1];
        }
    }
}
$internalHost = 'http://' . $internalIP . ':8081/';
