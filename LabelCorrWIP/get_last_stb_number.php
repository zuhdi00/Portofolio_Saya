...
try {
    ...
    $prefix = "WIP-" . $year . $month;
    $sql = "SELECT MAX(last_number) AS last_number FROM (
                SELECT MAX(CAST(RIGHT(cNoSTB, 4) AS INT)) AS last_number FROM tbWIPV2 WHERE cNoSTB LIKE ?
                UNION ALL
                SELECT MAX(CAST(RIGHT(cNoSTB, 4) AS INT)) AS last_number FROM tbTmpWIP WHERE cNoSTB LIKE ?
            ) x";
