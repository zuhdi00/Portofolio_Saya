...
$cSub4 = $input['cSub4'] ?? '';
$cSub5 = $input['cSub5'] ?? '';
// New field: pallet number (nIsi)
$nIsi = isset($input['nIsi']) ? intval($input['nIsi']) : 0;
// New fields: kepala shift, shift code, and shift date
$cKashift = $input['cKashift'] ?? '';
$cShift = $input['cShift'] ?? '';
$dTglShift = $input['dTglShift'] ?? '';
...
$dTglkirim = toSqlDate($dTglkirim);
// normalize dTglShift if provided
if ($dTglShift && !preg_match('/^\d{4}-\d{2}-\d{2}/', $dTglShift)) {
    $dTglShift = toSqlDate($dTglShift);
}
...
try {
    ...
    $sqlCheck = "SELECT COUNT(*) AS cnt FROM tbWIPV2 WHERE cNoSTB = ?";
    ...
    if ($rowCheck && $rowCheck['cnt'] > 0) {
        ...
        $sqlLast = "SELECT MAX(CAST(RIGHT(cNoSTB, 4) AS INT)) AS last_number FROM tbWIPV2 WHERE cNoSTB LIKE ?";
    ...
    $sql = "INSERT INTO tbTmpWIP (
        cNoSTB, dTanggal, cNoSc, cNoOp, cNama, cNamabrg, cNoMC, nPanjang, nLebar, nTinggi, nQty, nIsi, dTglkirim, lPosted, cNoid, lvoid, nQtyKg, cCom,
        cTipe, nBerat, cWarna, cJnsGel, cShift, cKashift, dTglShift, cSub1, cSub2, cSub3, cSub4, cSub5
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, '001', 0, ?, 'T', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    $params = [
        $cNoSTB, $dTanggal, $cNoSc, $cNoOp, $cNama, $cNamabrg, $cNoMC, $nPanjang, $nLebar,
        $nTinggi, $nQty, $nIsi, $dTglkirim, $nQtyKg, $cTipe, $nBerat, $cWarna, $cJnsGel, $cShift, $cKashift, $dTglShift, $cSub1,
        $cSub2, $cSub3, $cSub4, $cSub5
    ];
