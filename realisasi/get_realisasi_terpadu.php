...
$sql = "SELECT
    ...
    -- Serah Terima (aggregat dari tbStbBJ per OP)
    ISNULL(stb_agg.jml_stb, 0)          AS jml_serah_trm,
    stb_agg.tgl_serah,
    stb_agg.cRak,
    stb_agg.cShift,

    -- Pengiriman (aggregat dari tbSRJDtl per OP)
    ISNULL(srj_agg.jml_kirim, 0)        AS jml_kirim,
    srj_agg.tgl_kirim_srj,
    srj_agg.tujuan_kirim,
    ...
    ISNULL(conv_agg.hsl_conv, 0)        AS hsl_conv,
...
LEFT JOIN (
    SELECT d.cNoOp,
           SUM(ISNULL(d.nHasil,0) * ISNULL(d.nOut,1)) AS hsl_corr,
           SUM(ISNULL(d.nRusak,0))                     AS rsak_corr,
           SUM(ISNULL(cd.nQtyOrder,0))                 AS plan_corr
    FROM tbHslCorrDtl d
    LEFT JOIN tbCorrDtl cd ON cd.cNoCorr = d.cNoCorr AND cd.cNoOp = d.cNoOp
    GROUP BY d.cNoOp
...
LEFT JOIN (
    SELECT d.cNoOp,
           SUM(ISNULL(d.nHasil,0)) AS hsl_conv,
...
$cStmt = sqlsrv_query($conn, $countSql, empty($params) ? [] : $params);
$total = 0;
if ($cStmt) {
...
$sql = str_replace("SELECT", "SELECT TOP ($limit)", $sql, $c = 1);
