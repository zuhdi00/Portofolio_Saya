...
try {
    ...
    $sql = "SELECT 
                ...
                op.cTipe, op.nTot_netto, op.cWarna, op.cJnsGel,
                op.ckd_b1, op.ckd_b2, op.ckd_b3, op.ckd_b4, op.ckd_b5, op.cJnsGel, op.cFlexo, op.cKd_C, sc.nQty as sc_nQty, sc.cNoSC
            FROM tbOP op
            Inner Join tbSC sc ON op.cNoSc = sc.cNoSC
            WHERE op.cNoOp = ?";
    ...
    if ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        ...
        $row['sc_nQty'] = floatval($row['sc_nQty']);
