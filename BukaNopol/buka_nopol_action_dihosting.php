...
$targetUrl = "http://{$activeIP}:{$targetPort}/BukaNopol/buka_nopol_action.php";
if ($qs) {
    $targetUrl .= '?' . $qs;
}

// ─── FIX 7: Teruskan POST body ke PC lokal ──────────────────────────────────
// Masalah utama 400 Bad Request: proxy tidak mengirim $_POST ke PC.
// Bangun kembali POST body dari $_POST agar nopol, do, snapshot terkirim.
$isPost    = ($_SERVER['REQUEST_METHOD'] === 'POST');
$postBody  = '';
if ($isPost && !empty($_POST)) {
    // Encode sama persis seperti form HTML (application/x-www-form-urlencoded)
    $postBody = http_build_query($_POST);
} elseif ($isPost) {
    // Fallback: baca raw body (untuk Content-Type: application/json, dll.)
    $postBody = file_get_contents('php://input');
}

proxy_log("Proxy request -> method=" . $_SERVER['REQUEST_METHOD']
    . ", target={$targetUrl}"
...
curl_setopt_array($ch, [
    ...
    // ─── Teruskan method & body ──────────────────────────────────────────────
    CURLOPT_POST           => $isPost,
    CURLOPT_POSTFIELDS     => $postBody,  // string = application/x-www-form-urlencoded

    // ─── FIX 4: Timeout dalam detik ─────────────────────────────────────────
    ...
    // ─── FIX 6: User-Agent agar tidak diblokir firewall lokal ────────────────
    CURLOPT_USERAGENT      => 'ProxyDihosting/1.0',

    // ─── Kirim Content-Type yang benar ke PC ─────────────────────────────────
    CURLOPT_HTTPHEADER     => [
