...
BEGIN
    ...
    SET    b.nQtyOut     = k.keluar,
           b.cOutSTB     = 'KOREKSI STOK',
           b.dTanggalOut = k.tgl
    FROM   dbo.tbStbBJ b
    INNER JOIN kor k ON RTRIM(b.cNoSTB) = k.stb;

    /* Batch yang koreksinya dibatalkan seluruhnya, kembalikan ke nol */
    UPDATE b
    SET    b.nQtyOut = 0, b.cOutSTB = NULL, b.dTanggalOut = NULL
    FROM   dbo.tbStbBJ b
    WHERE  ISNULL(b.cOutSTB,'') = 'KOREKSI STOK'
    ...
    SELECT @@ROWCOUNT AS baris_disesuaikan;
END
GO
...
WHERE  ISNULL(cOutSTB,'') = 'KOREKSI STOK';
*/
