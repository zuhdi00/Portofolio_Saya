...
$departemenList = [];
$unitList = [];
$departmentResult = sqlsrv_query($conn, "SELECT id_dept, kode_dept, nama_dept FROM dbo.department ORDER BY nama_dept");
if ($departmentResult) {
    while ($row = sqlsrv_fetch_array($departmentResult, SQLSRV_FETCH_ASSOC)) {
        $departemenList[] = $row;
    }
}
$unitResult = sqlsrv_query($conn, "SELECT id, kode_unit, nama_unit, department_id FROM dbo.unit_kerja ORDER BY nama_unit");
if ($unitResult) {
    ...
    <section class="section">
        <form action="submit_pegawai_lengkap.php" method="POST" id="formPegawai">
            ...
            <fieldset>
                <div class="row g-3">
                    <div class="col-md-3">
                        ...
                        <input type="text" name="company_name" class="form-control" value="PT. SUPRACOR SEJAHTERA" required>
                    </div>
                    <div class="col-md-3">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="job_title" class="form-select">
                            ...
                            <option>Manager Produksi</option>
                            <option>Manager PPIC</option>
                            <option>Manager Marketing</option>
                            <option>Manager Accounting &amp; Finance</option>
                            <option>Manager HRGA</option>
                            <option>Senior Manager</option>
                            <option>Manager</option>
                            <option>Junior Manager</option>
                            <option>SPV Convert</option>
                            <option>SPV PPIC</option>
                            <option>SPV QC</option>
                            <option>SPV Gudang FG &amp; Ekspedisi</option>
                            <option>SPV Mekanik</option>
                            <option>SPV HR</option>
                            <option>SPV Accounting</option>
                            <option>SPV Corr</option>
                            <option>SPV Design &amp; Persiapan</option>
                            <option>Koordinator Design</option>
                            <option>Koordinator QC</option>
                            <option>Koordinator Corr</option>
                            <option>Koordinator Print</option>
                            <option>Koordinator Non Print</option>
                            <option>Koordinator Gudang Barang Jadi</option>
                            <option>Koordinator Gudang Barang Baku</option>
                            <option>Koordinator Gudang Barang Penolong</option>
                            <option>Koordinator Ekspedisi</option>
                            <option>Koordinator Mekanik</option>
                            <option>Koordinator Listrik</option>
                            <option>Koordinator Purchasing</option>
                            <option>Koordinator Internal Marketing</option>
                            <option>Koordinator EDP</option>
                            <option>Kepala Seksi (Kasie) Printing</option>
                            <option>Kepala Seksi Corr</option>
                            <option>Kepala Seksi Non Printing</option>
                            <option>Wakil Kepala Seksi (Wk. Kasie) Corr</option>
                            <option>Wakil Kepala Seksi (Wk. Kasie) Printing</option>
                            <option>Wakil Kepala Seksi (Wk. Kasie) Non Printing</option>
                            <option>Staff Design</option>
                            <option>Staff Produksi</option>
                            <option>Staff Purchasing</option>
                            <option>Staff Internal Marketing</option>
                            ...
                            <option>Staff Gudang Barang Jadi</option>
                            <option>Staff EDP</option>
                            <option>Admin Produksi</option>
                            <option>Admin QC</option>
                            <option>Admin Gudang Barang Jadi</option>
                            <option>Admin Gudang Barang Baku</option>
                            <option>Admin Gudang Barang Penolong</option>
                            <option>Admin Ekspedisi</option>
                            <option>Checker Gudang Barang Jadi</option>
                            <option>Checker Gudang Barang Baku</option>
                            <option>Checker Gudang Barang Penolong</option>
                            <option>Checker Ekspedisi</option>
                            <option>QC</option>
                            <option>Inspeksi QC</option>
                            <option>Inventory Control</option>
                            <option>Production Planning</option>
                            <option>R &amp; D</option>
                            <option>MC OP Data Analis</option>
                            <option>Operator Corr</option>
                            <option>Operator Print</option>
                            <option>Operator Non Print</option>
                            <option>Teknisi Listrik</option>
                            <option>Teknisi Mekanik</option>
                            <option>Sample Maker</option>
                            <option>APR Setting Dies &amp; Pisau</option>
                            <option>WIP</option>
                            <option>Helper Corr</option>
                            <option>Helper Printing</option>
                            <option>Helper Non Print</option>
                            <option>Helper Ekspedisi</option>
                            <option>Helper Persiapan</option>
                            <option>Helper Umum</option>
                            <option>Sopir</option>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="department_id" id="department_id" class="form-select" required>
                            ...
                            <?php foreach ($departemenList as $departemen): ?>
                                <option value="<?= (int)$departemen['id_dept'] ?>">
                                    <?= htmlspecialchars((string)$departemen['nama_dept']) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label" for="unit_kerja_id">Unit :</label>
                        <select name="unit_kerja_id" id="unit_kerja_id" class="form-select" required>
                            <option value="">-- pilih departemen terlebih dahulu --</option>
                            <?php foreach ($unitList as $unit): ?>
                                <option value="<?= (int)$unit['id'] ?>" data-department-id="<?= (int)($unit['department_id'] ?? 0) ?>">
                                    <?= htmlspecialchars((string)$unit['nama_unit']) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
            ...
            <fieldset>
                ...
                <div class="sub-box">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="hidden" name="bank_nama" value="PT. BANK CENTRAL ASIA">
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Bank Account :</label>
                            <input type="text" name="bank_rekening" class="form-control" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Nama Nasabah :</label>
                            <input type="text" name="nama_nasabah" class="form-control text-uppercase">
                        </div>
                ...
                <!-- Kontak Darurat -->
                <div class="sub-box">
                    <span class="box-title">Kontak Darurat</span>
                    <div class="row g-3">
                        <div class="col-md-3">
                            <label class="form-label">Nama :</label>
                            <input type="text" name="darurat_nama" class="form-control text-uppercase">
                        </div>
                        <div class="col-md-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" name="darurat_hp" class="form-control" placeholder="08xxxxxxxxxx">
                        </div>
                        <div class="col-md-12">
                            <label class="form-label">Alamat :</label>
                            <textarea name="darurat_alamat" class="form-control" rows="2"></textarea>
                        </div>
                    </div>
                </div>
            </fieldset>
                    ...
                    <table class="table table-bordered dyn-table" id="kelTable">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th>Birth Place</th><th>Birth Date</th><th></th>
                            </tr>
                        </thead>
                        <tbody></tbody>
                    </table>
...
function kelRow() {
    return `<td><input name="kel_nama[]" class="form-control form-control-sm text-uppercase"></td>
        ...
        <td><select name="kel_kawin[]" class="form-select form-select-sm"><option>SINGLE</option><option>MARRIED</option><option>DIVORCED</option></select></td>
...
document.getElementById('department_id').addEventListener('change', function () {
    ...
    Array.from(unitSelect.options).forEach(function (option, index) {
        if (index === 0) return;
        const sesuai = departmentId !== '' && option.dataset.departmentId === departmentId;
        option.hidden = !sesuai;
        option.disabled = !sesuai;
        if (sesuai) jumlahUnit++;
    });
    unitSelect.options[0].textContent = jumlahUnit
        ? '-- pilih unit --'
        : '-- unit departemen belum tersedia --';
});
