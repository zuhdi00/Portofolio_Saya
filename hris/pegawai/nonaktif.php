...
/* ---------- deteksi kolom opsional supaya query tidak gagal ---------- */
function adaKolom($conn, $tabel, $kolom) {
    $st = sqlsrv_query($conn,
        "SELECT 1 FROM sys.columns WHERE object_id=OBJECT_ID(?) AND name=?", [$tabel, $kolom]);
    $ada = $st ? (bool)sqlsrv_fetch_array($st, SQLSRV_FETCH_ASSOC) : false;
    if ($st) sqlsrv_free_stmt($st);
    return $ada;
}
// kandidat nama kolom tanggal keluar & alasan (beda-beda antar instalasi)
$kolTglKeluar = null;
foreach (['tgl_keluar','tanggal_keluar','tgl_berhenti','term_date','enterprise_end'] as $k) {
    if (adaKolom($conn,'dbo.pegawai',$k)) { $kolTglKeluar = $k; break; }
}
$kolAlasan = null;
foreach (['alasan_berhenti','term_reason','alasan_keluar'] as $k) {
    if (adaKolom($conn,'dbo.pegawai',$k)) { $kolAlasan = $k; break; }
}

$selTgl    = $kolTglKeluar ? "p.$kolTglKeluar AS tgl_keluar" : "NULL AS tgl_keluar";
$selAlasan = $kolAlasan    ? "p.$kolAlasan AS alasan_berhenti" : "NULL AS alasan_berhenti";
$orderBy   = $kolTglKeluar ? "p.$kolTglKeluar DESC, p.nama_peg" : "p.nama_peg";

$sqlList = "SELECT p.id_peg, p.nik, p.nama_peg, p.zkteco_userid, p.tgl_masuk,
                   $selTgl, $selAlasan, d.nama_dept
            FROM dbo.pegawai p
            LEFT JOIN dbo.unit_kerja u ON u.id = p.unit_kerja_id
            LEFT JOIN dbo.department d ON d.id_dept = u.department_id
            WHERE ".implode(' AND ',$where)."
            ORDER BY $orderBy";
$rows = sqlsrv_query($conn, $sqlList, $par);
if ($rows === false) {
    $e = sqlsrv_errors();
    $pesan .= "<div class='alert alert-danger'>Query gagal: "
            . htmlspecialchars($e[0]['message'] ?? 'tidak diketahui') . "</div>";
}
  ...
  <section class="section">
    ...
    <div class="card"><div class="card-body">
      ...
      <table class="table table-hover table-sm align-middle">
        ...
        <tbody>
        <?php $n=0; while($rows !== false && ($r=sqlsrv_fetch_array($rows,SQLSRV_FETCH_ASSOC))): $n++; ?>
          <tr>
            <td><?= h($r['nik']) ?></td>
