...
try {
    ...
    // Kontak darurat disimpan di tabel detail, bukan dbo.pegawai.
    if (v('darurat_nama') || v('darurat_hubungan') || v('darurat_hp') || v('darurat_alamat')) {
        $sqlDarurat = "INSERT INTO dbo.kontak_darurat
            (pegawai_id, nama, hubungan, no_hp, alamat) VALUES (?,?,?,?,?)";
        sqlsrv_free_stmt(q($conn, $sqlDarurat, [
            $idPeg,
            v('darurat_nama'),
            v('darurat_hubungan'),
            v('darurat_hp'),
    ...
    // ---------- 5. Pencatatan Histori ----------
    require_once __DIR__ . '/_catat_histori.php';
    [$namaUser, $idUser] = _userSekarang();
    
    $dataBaru = [];
    foreach ($map as $kolom => $field) {
        $dataBaru[$kolom] = enum_db('pegawai.' . $kolom, v($field));
    }
    $dataBaru['jabatan_id'] = v('jabatan_id') ? (int) v('jabatan_id') : null;
    $dataBaru['unit_kerja_id'] = v('unit_kerja_id') ? (int) v('unit_kerja_id') : null;
    $dataBaru['status_karyawan'] = enum_db('pegawai.status_karyawan', v('employee_subgroup')) ?? 'tetap';
    
    catatHistoriTambah($conn, $idPeg, $dataBaru, $namaUser, $idUser);

    sqlsrv_commit($conn);

    require_once __DIR__ . '/../config/_tulis_mdb.php';
    $mdbResult = tulisPegawaiKeMDB(
        (string) v('nama'),
        (string) v('nik'),
        (int) (v('department_id') ?: 22),
        (string) v('gender'),
        v('enterprise_begin')
    );
    $message = 'Data pegawai berhasil disimpan (ID: ' . $idPeg . '). ' . $mdbResult['pesan'];
