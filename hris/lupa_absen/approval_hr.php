...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if($id && in_array($keputusan,['setuju','tolak'],true)){
        ...
        try {
            ...
            // NOTIFIKASI balik ke pengaju
            $rsF=sqlsrv_query($conn,"SELECT no_form,dibuat_oleh FROM dbo.lupa_absen_form WHERE id_form=?",[$id]);
            if($ff=sqlsrv_fetch_array($rsF,SQLSRV_FETCH_ASSOC)){
                $rsU=sqlsrv_query($conn,"SELECT id_user FROM dbo.hris_users WHERE nama_lengkap=? AND is_aktif=1",[$ff['dibuat_oleh']]);
                $uids=[]; while($u2=sqlsrv_fetch_array($rsU,SQLSRV_FETCH_ASSOC)) $uids[]=(int)$u2['id_user'];
                if($uids){
                    $hasil=$keputusan==='setuju'?'DISETUJUI':'DITOLAK';
                    notifKeUser($conn,$uids,"Lupa Absen $hasil",
                        "Form {$ff['no_form']} telah $hasil oleh HRD.",'lupa_absen/input.php');
                }
            }
