...
$pegList=[]; $rs=sqlsrv_query($conn,"SELECT id_peg,nik,nama_peg FROM dbo.pegawai WHERE is_aktif=1 ORDER BY nama_peg");
...
<script>
const PEG = <?= json_encode(array_map(fn($p)=>['id'=>$p['id_peg'],'txt'=>$p['nik'].' — '.$p['nama_peg']], $pegList)) ?>;
function optionPeg(){
  return '<option value="">-- pilih --</option>' + PEG.map(p=>`<option value="${p.id}">${p.txt}</option>`).join('');
}
function addRow(){
  const tr=document.createElement('tr');
  tr.innerHTML=`
    <td><select name="peg[]" class="form-select form-select-sm">${optionPeg()}</select></td>
