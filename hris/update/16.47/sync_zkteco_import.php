...
// ================== KONFIGURASI ==================
$MDB_PATH   = 'D:\\ZKTeco\\Attendance\\ATT2000.mdb';  // <<< SESUAIKAN
$SUMBER     = 'ATT2000';
...
// ---------- 2. Buka MDB ----------
if (!file_exists($MDB_PATH)) { tulis("ERROR: file MDB tidak ditemukan: $MDB_PATH"); exit(1); }

try {
    $dsn = "odbc:Driver={Microsoft Access Driver (*.mdb, *.accdb)};Dbq={$MDB_PATH};Uid=;Pwd=;";
    $mdb = new PDO($dsn);
    $mdb->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
} catch (Exception $e) {
    tulis("ERROR koneksi MDB: " . $e->getMessage());
    tulis("Cek: Access Database Engine terpasang? bitness cocok dgn PHP? pdo_odbc aktif?");
    exit(1);
}
