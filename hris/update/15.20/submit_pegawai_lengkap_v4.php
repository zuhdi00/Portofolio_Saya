<?php
/**
 * submit_pegawai_lengkap.php   [VERSI v4 - 2026-07-23]
...
try {
    ...
    foreach ($map as $kolom => $field) {
        $cols[]   = $kolom;
        $params[] = enum_db($kolom, v($field));
    }

    // nilai default pegawai baru
    $cols[] = 'status_karyawan'; $params[] = enum_db('status_karyawan', 'Aktif');
    ...
    if (!empty($_POST['kel_nama'])) {
        ...
        foreach ($_POST['kel_nama'] as $i => $nm) {
            ...
            $p[] = $idPeg;
            $p[] = trim($nm);
            $p[] = $_POST['kel_hubungan'][$i] ?? null;
            $p[] = enum_db('gender', $_POST['kel_gender'][$i] ?? null);
            $p[] = enum_db('status_nikah', $_POST['kel_kawin'][$i] ?? null);
            $p[] = $_POST['kel_hidup'][$i] ?? null;
