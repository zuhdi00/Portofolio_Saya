...
$st = sqlsrv_query($conn,"SELECT * FROM dbo.pengunduran_diri WHERE id_resign=?",[$id]);
    ...
    if ($catatan==='') {
        ...
        sqlsrv_query($conn,
            "UPDATE dbo.pengunduran_diri
             SET atasan_nama=?, atasan_catatan=?, atasan_pada=GETDATE(), status='DISETUJUI'
             WHERE id_resign=?",
            [$nama?:null, $catatan, $id]);
    ...
    <div class="card"><div class="card-body">
      ...
      <?php if($r['status']==='DISETUJUI'): ?>
        <div class="alert alert-success">
          ...
          Catatan: <?= nl2br(h($r['atasan_catatan'])) ?>
        </div>
