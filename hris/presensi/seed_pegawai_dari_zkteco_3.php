...
foreach ($deptZk as $zid => $nama) {
    ...
    if ($TULIS) {
        ...
        $ins = sqlsrv_query($conn,
            "INSERT INTO dbo.department (kode_dept, nama_dept, sumber) VALUES (?,?,'ZKTECO');
...
if ($SEED_UNIT) {
    foreach ($petaDept as $namaUp => $idDept) {
        ...
        if ($TULIS) {
            $kode = 'U' . $idDept;
            $ins = sqlsrv_query($conn,
                "INSERT INTO dbo.unit_kerja (kode_unit, nama_unit, department_id, level, sumber)
                 VALUES (?,?,?,1,'ZKTECO');
    ...
    if ($TULIS) {
        $ins = sqlsrv_query($conn,
            "INSERT INTO dbo.pegawai (nik, nama_peg, gender, unit_kerja_id,
                                      zkteco_userid, zkteco_acno, is_aktif, company_name, sumber)
             VALUES (?,?,?,?,?,?,1,'GRP1','ZKTECO')",
