...
$MUNDUR_HARI = 1;     // mundur 1 hari: cukup menjaring shift malam (lintas tengah malam)
$BATAS_MUNDUR_HARI = 7;  // PENGAMAN: jangan pernah tarik lebih lama dari ini
                         //           (cegah sedot masal kalau last_checktime ke-reset)
// =================================================
...
// PENGAMAN: kalau titik mulai lebih lampau dari batas, paksa ke batas.
$batasMinimal = new DateTime();
$batasMinimal->modify("-{$BATAS_MUNDUR_HARI} days");
if ($sejak < $batasMinimal) {
    tulis("(!) Titik mulai (".$sejak->format('Y-m-d').") lebih lama dari batas {$BATAS_MUNDUR_HARI} hari.");
    tulis("    Dibatasi ke ".$batasMinimal->format('Y-m-d')." untuk cegah tarik masal.");
    tulis("    Kalau memang mau tarik historis, pakai tarik_historis_zkteco.php.");
    $sejak = $batasMinimal;
}
...
try {
    foreach ($rows as $r) {
        ...
        $ra = sqlsrv_rows_affected($ins);
        if ($ra > 0) $baru += $ra;   // -1 (baris sudah ada / dilewati) tidak dihitung
...
tulis("Sudah ada (dilewati): " . ($total - $baru) . " baris");
