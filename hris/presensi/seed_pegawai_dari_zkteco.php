...
// ================== KONFIGURASI ==================
$TULIS      = true;   // <<< false = uji coba, true = benar-benar menyimpan
...
function lengkapiWajib($wajib, &$cols, &$params) {
    // Kolom ini punya CHECK constraint - nilai kosong PASTI ditolak,
    // dan mengisinya dengan tebakan berbahaya (mis. gender salah).
    // Jalankan sql/07_longgarkan_kolom_wajib.sql supaya boleh NULL.
    $jangan = ['gender', 'status_nikah', 'status_karyawan', 'agama'];

    $ditambal = [];
    foreach ($wajib as $kol => $tipe) {
        if (in_array($kol, $cols, true)) continue;
        if (in_array($kol, $jangan, true)) {
            $GLOBALS['_kolom_bermasalah'][$kol] = true;
            continue;
        }
        $cols[]     = $kol;
...
$GLOBALS['_kolom_bermasalah'] = [];
$_cek = array_intersect(['gender','status_nikah','status_karyawan','agama'], array_keys($wajibPeg));
if ($_cek) {
    tulis("!! PERINGATAN: kolom ber-CHECK berikut masih NOT NULL: " . implode(', ', $_cek));
    tulis("   INSERT pegawai akan GAGAL. Jalankan dulu:");
    tulis("   sql/07_longgarkan_kolom_wajib.sql");
    tulis();
}
