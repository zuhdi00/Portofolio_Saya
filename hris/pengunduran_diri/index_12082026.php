    ...
    fetch(url)
      ...
      .then(function(data){
        ...
        box.innerHTML = data.map(p =>
          `<div class="saran-item" onmousedown="pilih(${p.id},'${(String(p.nik||'')+' \u2014 '+String(p.nama||'')).replace(/'/g,"\\\\'")}')"><strong>${p.nama}</strong> <span class="text-muted">${p.nik}</span></div>`
        ).join('');
        box.style.display = 'block';
      })
...
function filterPeg(){ document.getElementById('cariPeg').value=''; document.getElementById('pegId').value=''; document.getElementById('saranBox').style.display='none'; }
</script>
