...
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['aksi']??'')==='input') {
    // Cari pegawai dari teks "Nama — NIK" (datalist)
    $pegId = 0;
    $teks  = trim($_POST['peg_cari'] ?? '');
    if ($teks !== '') {
        // ambil NIK di belakang tanda em-dash
        $bagian = preg_split('/\s*—\s*/u', $teks);
        $nikCari = trim(end($bagian));
        if ($nikCari !== '') {
            $rsP = sqlsrv_query($conn, "SELECT TOP 1 id_peg FROM dbo.pegawai WHERE nik = ? AND is_aktif = 1", [$nikCari]);
            if ($rsP && ($rowP = sqlsrv_fetch_array($rsP, SQLSRV_FETCH_ASSOC))) $pegId = (int)$rowP['id_peg'];
            if ($rsP) sqlsrv_free_stmt($rsP);
        }
        // fallback: cocokkan nama persis
        if (!$pegId) {
            $namaCari = trim($bagian[0] ?? '');
            $rsP = sqlsrv_query($conn, "SELECT TOP 1 id_peg FROM dbo.pegawai WHERE nama_peg = ? AND is_aktif = 1", [$namaCari]);
            if ($rsP && ($rowP = sqlsrv_fetch_array($rsP, SQLSRV_FETCH_ASSOC))) $pegId = (int)$rowP['id_peg'];
            if ($rsP) sqlsrv_free_stmt($rsP);
        }
    }
    ...
    if (!$pegId) {
        $pesan = "<div class='alert alert-danger'>Karyawan tidak dikenali. Ketik lalu <strong>pilih dari daftar</strong> "
               . "yang muncul (format: Nama — NIK).</div>";
    } else {
        // ambil nama pegawai
        ...
        $st = sqlsrv_query($conn,
            "INSERT INTO dbo.pengunduran_diri
             (no_surat,pegawai_id,nama_pegawai,department_id,tanggal_pengajuan,tanggal_efektif,alasan,klasifikasi,dibuat_oleh,status)
             VALUES (?,?,?,?,?,?,?,?,?,'DIAJUKAN')",
            [$noSurat,$pegId,$namaPeg,$dept,$tglM?:null,$tglB?:null,$alasan?:null,$klasifikasi?:null,$u['nama_lengkap']??null]);
        $pesan = $st===false
...
/* ---------- edit klasifikasi (HRD/Admin IT) ---------- */
if ($_SERVER['REQUEST_METHOD']==='POST' && ($_POST['aksi']??'')==='ubah_klasifikasi') {
    $idU = (int)($_POST['id_resign'] ?? 0);
    $klU = trim($_POST['klasifikasi'] ?? '');
    $ctU = trim($_POST['klasifikasi_catatan'] ?? '');
    if ($idU) {
        $up = sqlsrv_query($conn,
            "UPDATE dbo.pengunduran_diri SET klasifikasi=?, klasifikasi_catatan=? WHERE id_pengunduran=?",
            [$klU?:null, $ctU?:null, $idU]);
        if ($up !== false) { sqlsrv_free_stmt($up);
            $pesan = "<div class='alert alert-success'>Klasifikasi diperbarui.</div>"; }
        else $pesan = "<div class='alert alert-danger'>Gagal memperbarui klasifikasi.</div>";
    }
}
...
// klasifikasi diambil dari master (dbo.klasifikasi_resign)
$KLASIFIKASI = [];
$rsK = @sqlsrv_query($conn, "SELECT kode,label FROM dbo.klasifikasi_resign WHERE is_aktif=1 ORDER BY urutan,kode");
if ($rsK) { while($k=sqlsrv_fetch_array($rsK,SQLSRV_FETCH_ASSOC)) $KLASIFIKASI[$k['kode']]=$k['label']; sqlsrv_free_stmt($rsK); }
if (!$KLASIFIKASI) $KLASIFIKASI = ['VOLUNTARY'=>'Voluntary','INVOLUNTARY'=>'Involuntary','LAINNYA'=>'Lainnya'];
...
<main id="main" class="main">
  <div class="pagetitle d-flex justify-content-between align-items-start flex-wrap">
    ...
    <div class="pt-2">
      <a href="daftar_penilaian.php" class="btn btn-outline-info btn-sm">
        <i class="bi bi-clipboard-data"></i> Penilaian</a>
      <a href="kelola_klasifikasi.php" class="btn btn-outline-secondary btn-sm">
        <i class="bi bi-tags"></i> Kelola Klasifikasi</a>
    </div>
  </div>
        ...
        <div class="card"><div class="card-body">
          ...
          <form method="POST">
            ...
            <div class="mb-2"><label class="form-label small">Karyawan</label>
              <input type="text" name="peg_cari" class="form-control" list="dlPeg" autocomplete="off"
                     placeholder="ketik nama atau NIK, lalu pilih dari daftar"
                     value="<?= h($_POST['peg_cari'] ?? '') ?>" required>
              <datalist id="dlPeg">
                <?php foreach($pegList as $p): ?>
                  <option value="<?= h($p['nama_peg'].' — '.$p['nik']) ?>"></option>
                <?php endforeach; ?>
              </datalist>
              <small class="text-muted">Ketik untuk mencari, lalu pilih dari daftar yang muncul.</small></div>
            ...
            <div class="mb-2"><label class="form-label small">Klasifikasi</label>
              ...
              <small><a href="kelola_klasifikasi.php" class="text-decoration-none">
                <i class="bi bi-gear"></i> kelola daftar klasifikasi</a></small></div>
        ...
        <div class="card"><div class="card-body">
          ...
          <table class="table table-hover align-middle">
            <thead class="table-light"><tr>
              <th>No Surat</th><th>Nama</th><th>Divisi</th><th>Berakhir</th><th>Klasifikasi</th><th>Status</th><th>Aksi</th>
            </tr></thead>
            <tbody>
              ...
              <tr>
                ...
                <td>
                  <form method="POST" class="d-flex gap-1">
                    <input type="hidden" name="aksi" value="ubah_klasifikasi">
                    <input type="hidden" name="id_resign" value="<?= $r['id_pengunduran'] ?>">
                    <select name="klasifikasi" class="form-select form-select-sm" style="min-width:130px">
                      <option value="">—</option>
                      <?php foreach($KLASIFIKASI as $k=>$lbl):
                        $sel = (($r['klasifikasi']??'')===$k)?'selected':''; ?>
                        <option value="<?= h($k) ?>" <?= $sel ?>><?= h($k) ?></option>
                      <?php endforeach; ?>
                    </select>
                    <button class="btn btn-sm btn-outline-primary" title="simpan klasifikasi"><i class="bi bi-save"></i></button>
                  </form>
                </td>
